    ...
    return `<div class="inv" onclick="openInvoiceDetail('${inv.id}')">
      <div class="inv-left">
        <div class="inv-top">
          ...
          <span class="inv-name">${inv.customer||'—'}</span>
        </div>
...
function ppickAddItem(pid, vid, silent){
  ...
  const isRA = store === 'ra';
  const price = isRA ? (p.priceRAPiece||0) : (p.priceFlora||0);
  ...
  row.innerHTML = `
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;align-items:end">
      ...
      <div>
        ...
        <input class="fi inv-item-price" type="number" value="${price.toFixed(2)}" step="0.01" style="font-size:14px;font-weight:700;text-align:center;padding:9px" oninput="recalcInvTotal()">
      </div>
      <div>
    ...
    <div style="margin-top:8px;background:var(--amber-soft);border-radius:10px;padding:8px 12px;display:flex;align-items:center;gap:10px">
      ...
      <input class="inv-dozen-inp" type="number" min="0" placeholder="0" style="width:52px;padding:5px 8px;border:1.5px solid var(--amber);border-radius:7px;font-size:13px;font-weight:700;text-align:center;background:white;color:var(--ink)" oninput="onDozenChange(this,'${pid}')">
...
function onDozenChange(dozenInp, pid){
...
function deleteInvoice(id){
  appConfirm('Delete Invoice','Delete this invoice? Stock will be restored if it was shipped.','🗑️ Delete',()=>{
    ...
    saveData(); renderInvoices(); renderInventory(); renderCustomers(); initDashboard();
...
function cancelInvoice(id){
  ...
  appConfirm('Cancel Invoice', `Cancel ${inv.num}? Stock will be restored to inventory.`, '✕ Cancel Invoice', ()=>{
    ...
    saveData(); closeModal('m-inv-detail'); renderInvoices(); renderInventory(); renderCustomers(); initDashboard();
...
function markInvoicePaid(id){
  ...
  saveData(); closeModal('m-inv-detail'); renderInvoices(); renderCustomers(); initDashboard();
...
async function renderShopOrders(){
  ...
  try{
    ...
    if(!data){ el.innerHTML = `<div style="text-align:center;padding:40px;color:var(--muted);font-size:14px">No orders yet 🛒<br><span style="font-size:12px">Orders placed from your shop will appear here</span></div>`; return; }
    const orders = Object.values(data).sort((a,b)=> (b.createdAt||'').localeCompare(a.createdAt||''));
    ...
    el.innerHTML = `<div style="text-align:center;padding:40px;color:var(--red);font-size:13px">❌ Failed to load orders<br><span style="font-size:11px">${e.message}</span></div>`;
  }
}

function _renderOrderCard(o){
  const storeIcon = o.store === 'flora' ? '🌸' : '🏪';
  const date = o.createdAt ? new Date(o.createdAt).toLocaleDateString('en',{day:'numeric',month:'short',hour:'2-digit',minute:'2-digit'}) : '—';
  const statusColors = { pending:'#f59e0b', confirmed:'#4caf7d', cancelled:'var(--muted)' };
  ...
  const statusLabel = statusLabels[o.status] || 'Pending';
  const itemsSummary = (o.items||[]).map(i=>`${i.name}${i.variantLabel?' ('+i.variantLabel+')':''} ×${i.qty}`).join(', ');

  const actionBtns = o.status === 'pending' ? `
    <div style="display:flex;gap:8px;margin-top:10px">
      <button onclick="confirmShopOrder('${o.id}')" style="flex:1;padding:9px;background:var(--rose);color:white;border:none;border-radius:10px;font-size:12px;font-weight:700;cursor:pointer;font-family:inherit">✅ Confirm → Invoice</button>
      <button onclick="cancelShopOrder('${o.id}')" style="flex:1;padding:9px;background:var(--grey2);color:var(--ink);border:none;border-radius:10px;font-size:12px;font-weight:700;cursor:pointer;font-family:inherit">✕ Cancel</button>
    </div>` : `
    <div style="display:flex;gap:8px;margin-top:10px">
      ${o.status==='confirmed'?`<button onclick="editShopOrder('${o.id}')" style="flex:1;padding:9px;background:var(--grey2);color:var(--ink);border:none;border-radius:10px;font-size:12px;font-weight:700;cursor:pointer;font-family:inherit">✏️ Edit Invoice</button>`:''}
      <button onclick="deleteShopOrder('${o.id}')" style="flex:1;padding:9px;background:var(--grey2);color:var(--red);border:none;border-radius:10px;font-size:12px;font-weight:700;cursor:pointer;font-family:inherit">🗑️ Delete</button>
    </div>`;

  return `<div style="background:white;border-radius:14px;padding:14px 16px;margin-bottom:10px;box-shadow:0 1px 6px rgba(0,0,0,0.07)">
    <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:6px">
      <div>
        <div style="font-size:14px;font-weight:700;color:var(--ink)">${storeIcon} ${o.customerName||'—'}</div>
        <div style="font-size:11px;color:var(--muted);margin-top:2px">#${o.id} · ${date}</div>
        ${o.customerPhone?`<div style="font-size:11px;color:var(--muted)">📞 ${o.customerPhone}</div>`:''}
        ${o.notes?`<div style="font-size:11px;color:var(--muted);font-style:italic">"${o.notes}"</div>`:''}
      </div>
      <div style="text-align:right">
        <div style="font-size:16px;font-weight:800;color:var(--ink)">$${(o.total||0).toFixed(2)}</div>
        <div style="font-size:10px;font-weight:700;color:${statusColor};margin-top:2px">${statusLabel}</div>
      </div>
    </div>
    <div style="font-size:11px;color:var(--muted);padding:8px 10px;background:var(--grey2);border-radius:8px;line-height:1.6">${itemsSummary}</div>
    ${actionBtns}
  </div>`;
}
...
async function confirmShopOrder(orderId){
  try{
    ...
    const invItems = (o.items||[]).map(i => {
      ...
      const variant = prod?.variants?.find(v => v.id === i.variantId) || prod?.variants?.[0];
      return {
    ...
    const newInv = {
      ...
      status: 'unpaid',
      paidAmt: 0,
      notes: `Shop order #${orderId}${o.notes?' — '+o.notes:''}`,
      items: invItems,
      total,
      stockReduced: false,
...
async function deleteShopOrder(orderId){
  ...
  try{
    await fetch(`${FB_ORDERS_URL}/${orderId}.json`,{method:'DELETE'});
