...
function setInvTab(tab){
  _invTab = tab;
  ['all','unpaid','confirmed','partial','shipped','paid','cancelled','orders'].forEach(t=>{
...
function renderInvoices(){
  ...
  const STATUS = { unpaid:{label:'Unpaid',cls:'br'}, confirmed:{label:'Confirmed ✅',cls:'bg'}, partial:{label:'Partial',cls:'ba'}, shipped:{label:'Shipped',cls:'bb'}, paid:{label:'Paid',cls:'bg'}, cancelled:{label:'Cancelled',cls:'bm'} };
  el.innerHTML = list.map(inv => {
    ...
    return `<div class="inv" onclick="openInvoiceDetail('${inv.id}')">
      ...
      ${inv.status==='cancelled'?`<button style="padding:7px 10px;font-size:11px;flex-shrink:0;background:var(--red-soft);color:var(--red);border:none;border-radius:8px;cursor:pointer;font-family:inherit" onclick="event.stopPropagation();deleteInvoice('${inv.id}')">🗑️</button>`:''}
    </div>`;
  }).join('');
}

function renderWholesale(){
  const raInvs = invoices.filter(i => (i.store === 'ra' || i.store === 'both') && i.status !== 'cancelled' && !i.fromShop);
      ...
      debtEl.innerHTML = debtList.map(d=>`
        <div class="lr">
          ...
          <div style="display:flex;flex-direction:column;align-items:flex-end;gap:4px">
            ...
            <button onclick="event.stopPropagation();window.open('https://wa.me/'+getStoreWA('ra').replace(/\D/g,'')+'?text='+encodeURIComponent('Hi '+d.name+'! Just a reminder about your outstanding balance of $'+d.owed.toFixed(2)+' 🌸'),'_blank')" style="background:#e8f8ef;color:#2e9960;border:none;border-radius:8px;padding:3px 8px;font-size:10px;font-weight:700;cursor:pointer;font-family:inherit">💬 WA</button>
          </div>
        </div>`).join('');
    }
  }
...
function _doSaveInvoice(andShare){
  ...
  // Auto-reduce stock when saved as confirmed, shipped or paid
  const isNew = !editId;
  if(isNew){
    if(status === 'confirmed' || status === 'shipped' || status === 'paid'){
    ...
    if(editedInv){
      const nowActive = status === 'confirmed' || status === 'shipped' || status === 'paid';
  ...
  if(savedInv?.fromShop && shopOrderRef){
    ...
    if(status==='paid' && savedInv.customerPhone){
      ...
      const storeLabel = savedInv.store==='flora' ? 'Flora Gift Shop' : 'RA Jemle LB';
      const waMsg = `Hi ${savedInv.customer||''}! 🌸\nYour order #${savedInv.num} has been paid & confirmed.\nThank you for shopping with ${storeLabel}! 💛`;
      setTimeout(()=>{
        appConfirm(
          '💬 Send Thank You?',
...
function deleteInvoice(id){
  appConfirm('Delete Invoice','Delete this invoice? Stock will be restored if applicable.','🗑️ Delete',()=>{
    const inv = invoices.find(i=>i.id===id);
    if(inv){
      raInvoiceRestoreStock(inv);
      const cust = customers.find(c=>c.name&&c.name.toLowerCase()===inv.customer?.toLowerCase());
      if(cust){
        if(inv.status==='unpaid'||inv.status==='confirmed') cust.debt = Math.max(0,(cust.debt||0)-inv.total);
...
function shareInvoiceWA(id){
  ...
  const custNum = wa.replace(/\D/g,'');
  const url = custNum ? `https://wa.me/${custNum}?text=${encodeURIComponent(lines)}` : `https://wa.me/?text=${encodeURIComponent(lines)}`;
...
function _renderOrderCard(o){
  ...
  const statusDot = { pending:'#f59e0b', confirmed:'var(--green)', shipped:'var(--blue,#5b8dee)', delivered:'var(--green)', cancelled:'var(--muted)' };
  ...
  const thumbs = (o.items||[]).slice(0,4).map(i => {
    ...
    const photo = prod?.photo || i.photo || '';
    const emoji = i.emoji || prod?.emoji || '📦';
    return photo
      ? `<div style="width:36px;height:36px;border-radius:8px;overflow:hidden;border:1.5px solid var(--grey2);flex-shrink:0"><img src="${photo}" style="width:100%;height:100%;object-fit:cover"></div>`
      : `<div style="width:36px;height:36px;border-radius:8px;background:var(--rose2);border:1.5px solid var(--grey2);display:flex;align-items:center;justify-content:center;font-size:16px;flex-shrink:0">${emoji}</div>`;
  }).join('');
  ...
  const itemList = isExpanded ? `
    <div style="margin-top:10px;border-top:1px solid var(--grey2);padding-top:10px;display:flex;flex-direction:column;gap:6px">
      ${(o.items||[]).map(i => {
        ...
        const emoji = i.emoji || prod?.emoji || '📦';
        const thumb = photo
          ? `<div style="width:40px;height:40px;border-radius:8px;overflow:hidden;flex-shrink:0"><img src="${photo}" style="width:100%;height:100%;object-fit:cover"></div>`
          : `<div style="width:40px;height:40px;border-radius:8px;background:var(--rose2);display:flex;align-items:center;justify-content:center;font-size:18px;flex-shrink:0">${emoji}</div>`;
  ...
  const actionBtns = isExpanded ? (() => {
    if(o.status === 'pending') return `
      <div style="display:flex;gap:8px;margin-top:12px;flex-wrap:wrap">
        <button onclick="event.stopPropagation();confirmShopOrder('${o.id}')" style="flex:1;min-width:120px;padding:9px;background:var(--rose);color:white;border:none;border-radius:10px;font-size:12px;font-weight:700;cursor:pointer;font-family:inherit">✅ Confirm</button>
        ...
        <button onclick="event.stopPropagation();cancelShopOrder('${o.id}')" style="padding:9px 12px;background:var(--grey2);color:var(--muted);border:none;border-radius:10px;font-size:12px;font-weight:700;cursor:pointer;font-family:inherit">✕ Cancel</button>
        <button onclick="event.stopPropagation();deleteShopOrder('${o.id}')" style="padding:9px 12px;background:var(--red-soft);color:var(--red);border:none;border-radius:10px;font-size:12px;font-weight:700;cursor:pointer;font-family:inherit">🗑️</button>
      </div>`;
    if(o.status === 'confirmed') return `
      <div style="display:flex;gap:8px;margin-top:12px;flex-wrap:wrap">
        <button onclick="event.stopPropagation();smUpdateOrderStatus('${o.id}','shipped')" style="flex:1;padding:9px;background:#e8f0ff;color:#4060c0;border:none;border-radius:10px;font-size:12px;font-weight:700;cursor:pointer;font-family:inherit">🚚 Mark Shipped</button>
        <button onclick="event.stopPropagation();editShopOrder('${o.id}')" style="padding:9px 12px;background:var(--grey2);color:var(--ink);border:none;border-radius:10px;font-size:12px;font-weight:700;cursor:pointer;font-family:inherit">📄 Invoice</button>
        ${waPhone?`<button onclick="event.stopPropagation();openCustomerWhatsApp('${o.customerPhone||''}')" style="padding:9px 12px;background:#e8f8ef;color:#2e9960;border:none;border-radius:10px;font-size:12px;font-weight:700;cursor:pointer;font-family:inherit">💬</button>`:''}
        <button onclick="event.stopPropagation();cancelShopOrder('${o.id}')" style="padding:9px 12px;background:var(--grey2);color:var(--muted);border:none;border-radius:10px;font-size:12px;font-weight:700;cursor:pointer;font-family:inherit">✕</button>
      </div>`;
    if(o.status === 'shipped') return `
      <div style="display:flex;gap:8px;margin-top:12px;flex-wrap:wrap">
        <button onclick="event.stopPropagation();smUpdateOrderStatus('${o.id}','delivered')" style="flex:1;padding:9px;background:var(--green-soft);color:var(--green);border:none;border-radius:10px;font-size:12px;font-weight:700;cursor:pointer;font-family:inherit">🎉 Mark Delivered</button>
        ${waPhone?`<button onclick="event.stopPropagation();openCustomerWhatsApp('${o.customerPhone||''}')" style="padding:9px 12px;background:#e8f8ef;color:#2e9960;border:none;border-radius:10px;font-size:12px;font-weight:700;cursor:pointer;font-family:inherit">💬</button>`:''}
        <button onclick="event.stopPropagation();editShopOrder('${o.id}')" style="padding:9px 12px;background:var(--grey2);color:var(--ink);border:none;border-radius:10px;font-size:12px;font-weight:700;cursor:pointer;font-family:inherit">📄</button>
      </div>`;
    if(o.status === 'delivered') return `
      <div style="display:flex;gap:8px;margin-top:12px">
        ${waPhone?`<button onclick="event.stopPropagation();openCustomerWhatsApp('${o.customerPhone||''}')" style="padding:9px 12px;background:#e8f8ef;color:#2e9960;border:none;border-radius:10px;font-size:12px;font-weight:700;cursor:pointer;font-family:inherit">💬 WhatsApp</button>`:''}
        <button onclick="event.stopPropagation();editShopOrder('${o.id}')" style="padding:9px 12px;background:var(--grey2);color:var(--ink);border:none;border-radius:10px;font-size:12px;font-weight:700;cursor:pointer;font-family:inherit">📄 Invoice</button>
        <button onclick="event.stopPropagation();deleteShopOrder('${o.id}')" style="padding:9px 12px;background:var(--red-soft);color:var(--red);border:none;border-radius:10px;font-size:12px;font-weight:700;cursor:pointer;font-family:inherit">🗑️</button>
      </div>`;
    // cancelled
    return `
      <div style="display:flex;gap:8px;margin-top:12px">
        <button onclick="event.stopPropagation();deleteShopOrder('${o.id}')" style="flex:1;padding:9px;background:var(--red-soft);color:var(--red);border:none;border-radius:10px;font-size:12px;font-weight:700;cursor:pointer;font-family:inherit">🗑️ Delete Order</button>
        ${waPhone?`<button onclick="event.stopPropagation();openCustomerWhatsApp('${o.customerPhone||''}')" style="padding:9px 12px;background:#e8f8ef;color:#2e9960;border:none;border-radius:10px;font-size:12px;font-weight:700;cursor:pointer;font-family:inherit">💬</button>`:''}
      </div>`;
  })() : '';
...
async function confirmShopOrder(orderId){
  try{
    ...
    // Debt will be calculated via recalcDebt — don't double add here
...
async function cancelShopOrder(orderId){
  appConfirm('Cancel Order', 'Cancel this order? Stock will be restored if it was confirmed.', '✕ Cancel', async ()=>{
    try{
