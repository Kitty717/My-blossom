...
function showQR(pid){
  ...
  // Build variant QR section
  const hasVariants = p.variants && p.variants.length > 1;
  const variantSection = hasVariants ? `
    <div style="margin-top:14px;padding-top:14px;border-top:1px solid var(--grey2)">
      <div style="font-size:10px;font-weight:700;color:var(--muted);text-transform:uppercase;letter-spacing:1px;margin-bottom:10px">📦 Per-Variant QR Codes</div>
      <div style="display:flex;flex-direction:column;gap:8px" id="variant-qr-list"></div>
      <button class="btn btn-g btn-full" style="margin-top:10px;font-size:12px" onclick="printAllVariantQRs('${pid}')">🖨️ Print All Variant Labels</button>
    </div>` : '';
  ...
  // Inject variant section
  const existingVarSection = document.getElementById('qr-variant-section');
  if(existingVarSection) existingVarSection.remove();
  if(hasVariants){
    const container = document.getElementById('qr-state-generate').parentNode;
    const div = document.createElement('div');
    div.id = 'qr-variant-section';
    div.innerHTML = variantSection;
    container.appendChild(div);
    // Generate mini QRs for each variant
    setTimeout(()=>{
      const list = document.getElementById('variant-qr-list');
      if(!list) return;
      p.variants.forEach(v=>{
        const label = [v.label, v.name&&v.name!=='Standard'?v.name:'', v.size].filter(Boolean).join(' · ') || 'Standard';
        const sku = v.barcode || ('BLM-'+p.id+'|'+v.id);
        const row = document.createElement('div');
        row.style.cssText = 'display:flex;align-items:center;gap:12px;padding:8px;background:var(--grey);border-radius:10px';
        const canvasWrap = document.createElement('div');
        canvasWrap.id = 'vqr-'+v.id;
        canvasWrap.style.cssText = 'width:60px;height:60px;flex-shrink:0';
        const info = document.createElement('div');
        info.style.cssText = 'flex:1;min-width:0';
        info.innerHTML = `<div style="font-size:12px;font-weight:700;color:var(--ink)">${label}</div>
          <div style="font-size:10px;color:var(--muted);margin-top:2px">${sku}</div>
          ${v.pricePiece?`<div style="font-size:10px;color:var(--amber);font-weight:600">$${v.pricePiece.toFixed(2)}/pc</div>`:''}
          ${v.barcode?`<div style="font-size:9px;color:var(--green);font-weight:600">📦 Custom barcode</div>`:'<div style="font-size:9px;color:var(--muted)">Auto SKU</div>'}`;        const printBtn = document.createElement('button');
        printBtn.textContent = '🖨️';
        printBtn.style.cssText = 'background:var(--rose);border:none;border-radius:8px;padding:6px 10px;cursor:pointer;font-size:14px;flex-shrink:0';
        printBtn.onclick = ()=> printVariantQRLabel(p, v, sku);
        row.appendChild(canvasWrap);
        row.appendChild(info);
        row.appendChild(printBtn);
        list.appendChild(row);
        new QRCode(canvasWrap, {text:sku, width:60, height:60, colorDark:'#2c1a1f', colorLight:'#ffffff', correctLevel:QRCode.CorrectLevel.H});
      });
    }, 100);
  }
  showModal('m-qr');
...
function printVariantQRLabel(p, v, sku){
  const label = [v.label, v.name&&v.name!=='Standard'?v.name:'', v.size].filter(Boolean).join(' · ') || 'Standard';
  const priceStr = v.pricePiece ? '$'+v.pricePiece.toFixed(2)+'/pc' : (p.priceRAPiece?'$'+p.priceRAPiece.toFixed(2)+'/pc':'');
  const wrap = document.getElementById('vqr-'+v.id);
  const canvas = wrap ? wrap.querySelector('canvas') : null;
  // Generate a bigger canvas for print
  const tempDiv = document.createElement('div');
  tempDiv.style.cssText = 'position:fixed;top:-9999px;left:-9999px';
  document.body.appendChild(tempDiv);
  new QRCode(tempDiv, {text:sku, width:200, height:200, colorDark:'#2c1a1f', colorLight:'#ffffff', correctLevel:QRCode.CorrectLevel.H});
  setTimeout(()=>{
    const bigCanvas = tempDiv.querySelector('canvas');
    const imgData = bigCanvas ? bigCanvas.toDataURL('image/png') : '';
    document.body.removeChild(tempDiv);
    const w = window.open('','_blank','width=400,height=340');
    if(!w){ showToast('Allow popups to print','err'); return; }
    w.document.write(`<!DOCTYPE html><html><head><title>Variant Label</title>
    <style>body{margin:0;display:flex;align-items:center;justify-content:center;min-height:100vh;background:#fff;font-family:monospace}
    .label{border:2px dashed #ccc;border-radius:12px;padding:16px 24px;text-align:center;display:inline-block}
    .label img{width:160px;height:160px;display:block;margin:0 auto 8px}
    .pname{font-size:13px;font-weight:700;color:#2c1a1f;margin-bottom:2px}
    .vname{font-size:12px;color:#555;margin-bottom:2px}
    .price{font-size:13px;font-weight:700;color:#c9900a;margin-bottom:2px}
    .sku{font-size:9px;color:#999;letter-spacing:0.5px}
    @media print{body{margin:0}.label{border:none;padding:8px}}</style>
...
function printAllVariantQRs(pid){
  const p = products.find(x=>x.id===pid);
  if(!p||!p.variants) return;
  const labels = p.variants.map(v=>{
    const label = [v.label, v.name&&v.name!=='Standard'?v.name:'', v.size].filter(Boolean).join(' · ') || 'Standard';
    const sku = 'BLM-'+p.id+'|'+v.id;
    const priceStr = v.pricePiece ? '$'+v.pricePiece.toFixed(2) : (p.priceRAPiece?'$'+p.priceRAPiece.toFixed(2):'');
    return {label, sku, priceStr, v};
  });
  // Generate QRs sequentially then print
  const w = window.open('','_blank','width=600,height=500');
  if(!w){ showToast('Allow popups to print','err'); return; }
  w.document.write(`<!DOCTYPE html><html><head><title>All Variant Labels — ${p.name}</title>
  <style>body{margin:16px;font-family:monospace;background:#fff}
  .grid{display:flex;flex-wrap:wrap;gap:12px}
  .label{border:2px dashed #ccc;border-radius:10px;padding:12px 16px;text-align:center;display:inline-block;width:160px}
  .label canvas{display:block;margin:0 auto 6px}
  .pname{font-size:10px;font-weight:700;color:#2c1a1f;margin-bottom:1px}
  .vname{font-size:10px;color:#555;margin-bottom:1px}
  .price{font-size:10px;font-weight:700;color:#c9900a}
  .sku{font-size:8px;color:#aaa;margin-top:2px}
  @media print{body{margin:4px}.grid{gap:6px}.label{border:1px dashed #ccc;padding:8px;width:140px}}</style>
  <script src="https://cdnjs.cloudflare.com/ajax/libs/qrcodejs/1.0.0/qrcode.min.js"><\/script>
  </head><body>
  <div style="font-size:14px;font-weight:700;margin-bottom:12px">${p.emoji||''} ${p.name} — All Variant Labels</div>
  <div class="grid">
  ${labels.map((l,i)=>`<div class="label">
    <div id="qr${i}"></div>
    <div class="pname">${p.emoji||''} ${p.name}</div>
    <div class="vname">${l.label}</div>
    ${l.priceStr?`<div class="price">${l.priceStr}</div>`:''}
    <div class="sku">${l.sku}</div>
  </div>`).join('')}
  </div>
  <script>
  window.onload=function(){
    ${labels.map((l,i)=>`new QRCode(document.getElementById('qr${i}'),{text:'${l.sku}',width:120,height:120,colorDark:'#2c1a1f',colorLight:'#fff',correctLevel:QRCode.CorrectLevel.H});`).join('\n')}
...
function startScan(mode){
  scanMode=mode;
  const label = (mode==='np-barcode-fill'||mode==='ep-barcode-fill'||mode.startsWith('var-barcode-fill:')) ? 'Scan Variant Barcode' : mode==='add' ? 'Scan to Add Product' : mode==='import' ? 'Scan to Import Product' : (mode==='inv-add'||mode==='inv-add-flora') ? 'Scan to Add to Invoice' : 'Scan to Update Stock';
...
function handleScan(code){
  ...
  // Variant barcode fill: mode = 'var-barcode-fill:uid'
  if(scanMode && scanMode.startsWith('var-barcode-fill:')){
    const targetId = scanMode.replace('var-barcode-fill:','');
    setTimeout(()=>{
      document.getElementById('scan-overlay').classList.remove('active');
      const hiddenInp = document.getElementById(targetId+'-barcode');
      if(hiddenInp){
        // Find the closest var-barcode-inp in the same variant row
        const row = hiddenInp.closest ? hiddenInp.closest('.variant-input-row') : null;
        const barcodeInp = row ? row.querySelector('.var-barcode-inp') : null;
        if(barcodeInp){ barcodeInp.value=code; showToast('Variant barcode captured ✅'); }
      }
    },400);
    return;
  }

  // Parse BLM QR — supports both product-level (BLM-pid) and variant-level (BLM-pid|vid)
  const parts = code.split('|');
  const rawPid = parts[0].replace(/^BLM-/i,'');
  const rawVid = parts[1] || null;
  ...
  // If variant id present in QR, find it
  const v = rawVid && p ? (p.variants.find(x=>x.id===rawVid)||p.variants[0]) : (p?.variants[0]);
  setTimeout(()=>{
    document.getElementById('scan-overlay').classList.remove('active');
    if(scanMode==='import'){
      if(p){
