...
function saveProducts(){
  ...
  // Save photos that are in memory to IDB_PHOTOS (fire and forget per photo)
  products.forEach(p => {
    if(p.photo && p.photo.startsWith('data:')) savePhoto(p.id, p.photo).catch(()=>{});
    (p.variants||[]).forEach(v => {
      if(v.photo && v.photo.startsWith('data:')) savePhoto(p.id+'__v__'+v.id, v.photo).catch(()=>{});
    });
  });
