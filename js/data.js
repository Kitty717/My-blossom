...
function saveProducts(){
  ...
  // Also persist any photos that are in memory to IDB_PHOTOS
  products.forEach(p => {
    if(p.photo && p.photo.startsWith('data:')) savePhoto(p.id, p.photo);
    (p.variants||[]).forEach(v => {
      if(v.photo && v.photo.startsWith('data:')) savePhoto(p.id+'__v__'+v.id, v.photo);
    });
  });
