...
function saveTodo(){
  ...
  saveData(); renderTodos(); initDashboard();
...
function toggleTodo(id){
  ...
  if(t){ t.done=!t.done; saveData(); renderTodos(); initDashboard(); }
}

function pinTodo(id){
  const t=todos.find(x=>x.id===id);
  if(t){ t.pinned=!t.pinned; saveData(); renderTodos(); showToast(t.pinned?'Pinned 📌':'Unpinned'); }
}
...
function deleteTodo(id){
  ...
  if(i>-1){
    todos.splice(i,1);
    // If this is an auto-generated reorder todo, dismiss the reorder alert
    // so Smart Alerts doesn't immediately recreate it
    if(t && t.id.startsWith('reorder-') && t.linkId){
      const d = (()=>{ try{ return JSON.parse(localStorage.getItem('biz_reorder_dismissed')||'{}'); }catch(e){ return {}; } })();
