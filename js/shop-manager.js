  ...
  el.innerHTML = `
    ...
    <div class="card" style="margin-bottom:14px">
      ...
      <div style="margin-bottom:14px">
        ...
        <div style="font-size:11px;color:var(--muted);margin-top:4px">Leave blank to hide. Shows as a banner at the top of your shop.</div>
      </div>
      <!-- Coupon Codes -->
      <div style="margin-bottom:18px">
        <span class="fl">🎟️ Coupon Codes</span>
        <div style="font-size:11px;color:var(--muted);margin-bottom:8px">One per line · Format: CODE=10% or CODE=5 (fixed $)</div>
        <textarea class="fi" id="ss-coupons" placeholder="WELCOME10=10%&#10;VIP=5&#10;SARA=15%" style="width:100%;min-height:80px;resize:vertical;font-size:12px;line-height:1.7">${(shopSettings.coupons||[]).map(c=>c.code+'='+(c.type==='percent'?c.value+'%':c.value)).join('\n')}</textarea>
      </div>
      <div style="margin-bottom:18px">
...
function renderShopProductList() {
  ...
  return products.map(p => {
    ...
    const hasPrice   = piecePrice > 0 || dozenPrice > 0 || !!su;
      ...
      <div style="display:flex;align-items:center;gap:12px;padding:12px 0;border-bottom:1px solid var(--grey2)">
        ...
        <div style="flex:1;min-width:0">
          ...
          <div style="font-size:11px;color:var(--muted);display:flex;align-items:center;gap:6px;margin-top:2px;flex-wrap:wrap">
            ...
            ${hasPrice ? `<span>·</span><span>${piecePrice?'$'+piecePrice.toFixed(2)+'/pc':''}${dozenPrice?' · $'+(dozenPrice*12).toFixed(2)+'/doz':''}${su?' · '+su.name+' $'+su.price.toFixed(2)+'('+su.qty+'pc)':''}</span>` : '<span style="color:var(--red)">· No price set</span>'}
...
async function saveShopSettingsForm() {
  ...
  // Parse coupon codes: CODE=10% or CODE=5
  const couponRaw = (document.getElementById('ss-coupons').value||'').split('\n');
  shopSettings.coupons = couponRaw.map(l=>l.trim()).filter(Boolean).map(l=>{
    const [code, val] = l.split('=');
    if(!code||!val) return null;
    const isPercent = val.trim().endsWith('%');
    const num = parseFloat(val);
    if(!num) return null;
    return { code: code.trim().toUpperCase(), type: isPercent?'percent':'fixed', value: num };
  }).filter(Boolean);
  await saveShopSettings();
}
