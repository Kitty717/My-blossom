...
const SHOP_URL  = 'https://kitty717.github.io/My-blossom/shop.html';
const FB_DB_URL = 'https://ra-shop-3e01d-default-rtdb.firebaseio.com';

// ── Compress image to max 400px / 70% quality ──
async function compressImage(base64, maxDim = 400, quality = 0.7) {
...
async function publishShopToFirebase() {
  ...
  try {
    const publishProducts = await Promise.all(
    ...
    const res = await fetch(`${FB_DB_URL}/shop.json`, {
      method: 'PUT',
    ...
    if (statusEl) statusEl.innerHTML = `<span style="color:#4caf7d;font-size:12px">✅ Last published: ${new Date().toLocaleString()}</span>`;
  } catch(e) {
    console.error('publishShopToFirebase', e);
      ...
      <div style="display:flex;align-items:center;gap:12px;padding:12px 0;border-bottom:1px solid var(--grey2)">
        ...
        <div style="flex:1;min-width:0">
          ...
          <div style="font-size:11px;color:var(--muted);display:flex;align-items:center;gap:6px;margin-top:2px;flex-wrap:wrap">
            ...
            ${hasPrice ? `<span>·</span><span>${piecePrice?'$'+piecePrice.toFixed(2)+'/pc':''}${dozenPrice?' · $'+(dozenPrice/12).toFixed(2)+'/pc · $'+dozenPrice.toFixed(2)+'/doz':''}</span>` : '<span style="color:var(--red)">· No price set</span>'}
...
function copyShopLink() {
  navigator.clipboard.writeText(SHOP_URL)
...
function shareShopLink() {
  if (navigator.share) {
    navigator.share({ title: shopSettings.shopName||'My Shop', url: SHOP_URL }).catch(() => copyShopLink());
  } else {
    copyShopLink();
  }
}
