...
function openManualRecovery(sid){
  ...
  document.getElementById('manual-recovery-ov')?.remove();
  const ov = document.createElement('div');
  ov.id = 'manual-recovery-ov';
  ov.style.cssText='position:fixed;inset:0;background:rgba(44,26,31,0.6);z-index:99999;display:flex;align-items:flex-end;justify-content:center;backdrop-filter:blur(2px)';
  ov.innerHTML=`<div style="background:white;border-radius:24px 24px 0 0;width:100%;max-width:480px;padding:24px 20px 36px;font-family:'DM Sans',sans-serif">
    <div style="width:36px;height:4px;background:rgba(26,10,16,0.12);border-radius:4px;margin:0 auto 20px"></div>
    <div style="font-size:16px;font-weight:700;color:#2c1a20;margin-bottom:4px">💰 Capital Recovery</div>
    <div style="font-size:12px;color:#9a8078;margin-bottom:14px">${s.name}</div>
    <div style="background:#f5f0f2;border-radius:12px;padding:12px 14px;margin-bottom:16px;display:flex;justify-content:space-between;align-items:center">
      <span style="font-size:13px;color:#9a8078">Shipment cost</span>
      <span style="font-size:14px;font-weight:700;color:#2c1a20">$${cost.toLocaleString()}</span>
    </div>
    <div style="font-size:11px;font-weight:700;color:#9a8078;text-transform:uppercase;letter-spacing:1px;margin-bottom:8px">How much did you collect from this shipment?</div>
    <input id="manual-recovery-inp" type="number" step="0.01" value="${current||''}" placeholder="${cost}"
      style="width:100%;padding:14px;border:1.5px solid #ede0d8;border-radius:12px;font-family:'DM Sans',sans-serif;font-size:20px;font-weight:700;color:#2c1a20;outline:none;margin-bottom:8px;background:#faf4ee">
    <div id="recovery-hint" style="font-size:12px;color:#9a8078;margin-bottom:20px;min-height:18px;text-align:center"></div>
    <div style="display:flex;gap:10px">
      <button onclick="document.getElementById('manual-recovery-ov').remove()" style="flex:1;padding:14px;background:#f5f0f2;border:none;border-radius:12px;font-family:'DM Sans',sans-serif;font-size:13px;font-weight:600;cursor:pointer;color:#9a8078">Cancel</button>
      <button onclick="saveManualRecovery('${sid}',document.getElementById('manual-recovery-inp').value)" style="flex:2;padding:14px;background:linear-gradient(135deg,#2a8a56,#38b275);border:none;border-radius:12px;font-family:'DM Sans',sans-serif;font-size:13px;font-weight:700;cursor:pointer;color:white">💰 Save</button>
    </div>
  </div>`;
  // Live hint
  ov.querySelector('#manual-recovery-inp').addEventListener('input', function(){
    ...
    if(val>=cost) hint.innerHTML=`<span style="color:#2a8a56;font-weight:700">✅ Capital fully recovered! Profit: $${(val-cost).toFixed(2)}</span>`;
    else hint.innerHTML=`<span style="color:#b07800;font-weight:700">⚠️ ${Math.round((val/cost)*100)}% recovered · Still need $${(cost-val).toFixed(2)}</span>`;
  });
  // Trigger hint if pre-filled
  if(current) ov.querySelector('#manual-recovery-inp').dispatchEvent(new Event('input'));
