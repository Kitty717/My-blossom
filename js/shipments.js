...
function markShipArrived(sid){
  ...
  // Navigate to Stock Count page to show confirmation
  setTimeout(()=>{ showPage('inventory'); setNav('more'); renderCount(); }, 800);
}
...
function openShipChecklist(sid){
  ...
  if(!linked.length){
    appConfirm('No Products', 'No products linked to this shipment. Mark as arrived anyway?', '✅ Mark Arrived', ()=>{
      s.status='arrived'; saveData(); closeModal('m-ship-detail'); renderShipments(); initDashboard(); showToast('✅ Shipment arrived!');
    });
    return;
  }
  _checklistItems = [];
...
function cfaAnswer(pid, vid, allArrived){
  ...
  if(allArrived){
    ...
    saveData(); renderInventory(); checkAllConfirmed(pid);
  } else {
    // Show qty input
    stepEl.innerHTML = `
      <div style="font-size:11px;font-weight:700;color:var(--red);margin-bottom:6px">How many actually arrived?</div>
...
function confirmShipChecklist(){
  ...
  saveData(); closeModal('m-ship-checklist'); renderShipments(); renderInventory(); initDashboard();
}
...
function convertRefundToLoss(sid, refId){
  ...
  appConfirm('Convert to Loss','Supplier won\'t refund? Log $'+ref.cost.toFixed(2)+' as a loss?','💸 Convert',()=>{
    ...
    saveData(); renderShipments(); initDashboard(); showToast('💸 Logged as loss');
  });
}
...
function autoExpireRefunds(){
  ...
  if(expired>0){ saveData(); if(typeof initDashboard==='function') initDashboard(); }
}
