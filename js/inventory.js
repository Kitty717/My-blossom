...
//   showQR(), setCatChipValue(), injectCustomCatChip(),
//   isProductInTransit(), openVColorPicker()
// ═══════════════════════════════════════════════════

// ── Variant Card Builder ──
function _buildVariantCardHTML(uid, store, opts={}){
  const { label='', name='', size='', colorHex='#ede6e8', hasPhoto=false, photoSrc='',
          ra=0, flora=0, priceDozen='', sellingPrice='', barcode='', isEdit=false } = opts;
  const qtyBothRA = isEdit ? 'ep-qty-ra'     : 'np-qty-ra';
  const qtyBothFL = isEdit ? 'ep-qty-fl'     : 'np-qty-fl';
  const qtySingle = isEdit ? 'ep-qty-single' : 'np-qty-single';
  const qtyVal    = store==='flora' ? (flora||'') : (ra||'');
  const qtyHtml = store==='both'
    ? `<div style="display:grid;grid-template-columns:1fr 1fr;gap:6px">
         <div>
           <div style="font-size:9px;font-weight:700;color:var(--amber);text-transform:uppercase;margin-bottom:3px">🏪 RA Qty</div>
           <input class="fi ${qtyBothRA}" type="number" min="0" value="${ra||''}" placeholder="0" style="font-size:15px;font-weight:700;text-align:center;padding:9px;width:100%;border-color:var(--amber)">
         </div>
         <div>
           <div style="font-size:9px;font-weight:700;color:var(--rose);text-transform:uppercase;margin-bottom:3px">🌸 Flora Qty</div>
           <input class="fi ${qtyBothFL}" type="number" min="0" value="${flora||''}" placeholder="0" style="font-size:15px;font-weight:700;text-align:center;padding:9px;width:100%;border-color:var(--rose-light)">
         </div>
       </div>`
    : `<div>
         <div style="font-size:9px;font-weight:700;color:var(--rose);text-transform:uppercase;margin-bottom:3px">${store==='flora'?'🌸':'🏪'} Qty</div>
         <input class="fi ${qtySingle}" type="number" min="0" value="${qtyVal}" placeholder="0" style="font-size:15px;font-weight:700;text-align:center;padding:9px;width:100%;border-color:var(--rose-light)">
       </div>`;
  return `
    <div style="display:grid;grid-template-columns:1fr 1fr 0.8fr;gap:8px;margin-bottom:10px">
      <div>
        <div style="font-size:9px;font-weight:700;color:var(--muted);text-transform:uppercase;margin-bottom:3px">Type</div>
        <input class="fi var-label-inp" value="${label}" placeholder="e.g. Shade" style="font-size:13px;padding:9px;width:100%">
      </div>
      <div>
        <div style="font-size:9px;font-weight:700;color:var(--muted);text-transform:uppercase;margin-bottom:3px">Value</div>
        <input class="fi var-name-inp" value="${name}" placeholder="e.g. Pink" style="font-size:13px;padding:9px;width:100%">
      </div>
      <div>
        <div style="font-size:9px;font-weight:700;color:var(--muted);text-transform:uppercase;margin-bottom:3px">Size</div>
        <input class="fi var-size-inp" value="${size}" placeholder="30ml" style="font-size:13px;padding:9px;width:100%">
      </div>
    </div>
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:10px">
      <div>
        <div style="font-size:9px;font-weight:700;color:var(--green);text-transform:uppercase;margin-bottom:3px">💵 Selling Price</div>
        <input class="fi var-selling-price-inp" type="number" step="0.01" value="${sellingPrice||''}" placeholder="e.g. 5.00" style="font-size:14px;font-weight:700;padding:9px;width:100%;border-color:rgba(76,175,125,0.4)">
      </div>
      <div>
        <div style="font-size:9px;font-weight:700;color:var(--amber);text-transform:uppercase;margin-bottom:3px">💲 Dozen Override</div>
        <input class="fi var-price-dozen-inp" type="number" step="0.01" value="${priceDozen||''}" placeholder="blank = default" style="font-size:13px;padding:9px;width:100%">
      </div>
    </div>
    <div style="margin-bottom:10px">${qtyHtml}</div>
    <div style="display:flex;gap:8px;align-items:flex-end">
      <div style="flex:1">
        <div style="font-size:9px;font-weight:700;color:var(--muted);text-transform:uppercase;margin-bottom:3px">📦 Barcode</div>
        <div style="display:flex;gap:6px;align-items:center">
          <input class="fi var-barcode-inp" value="${barcode}" placeholder="Scan or type" style="font-size:12px;padding:8px;flex:1;min-width:0">
          <button type="button" onclick="startScan('var-barcode-fill:${uid}')" style="background:var(--rose);color:white;border:none;border-radius:10px;padding:8px 10px;font-size:13px;cursor:pointer;flex-shrink:0">📷</button>
          <input type="hidden" id="${uid}-barcode">
        </div>
      </div>
      <div style="flex-shrink:0">
        <div style="font-size:9px;font-weight:700;color:var(--muted);text-transform:uppercase;margin-bottom:3px;text-align:center">🎨</div>
        <input type="hidden" class="variant-color-input" value="${colorHex}">
        <button type="button" class="vcolor-btn" onclick="openVColorPicker(this)" style="width:42px;height:42px;border-radius:50%;background:${colorHex};border:2px solid var(--grey2);cursor:pointer;display:block;box-shadow:0 2px 8px rgba(0,0,0,0.12)" title="Pick color"></button>
      </div>
      <div style="flex-shrink:0">
        <div style="font-size:9px;font-weight:700;color:var(--muted);text-transform:uppercase;margin-bottom:3px;text-align:center">📷</div>
        <div style="position:relative;width:42px;height:42px">
          ...
          <div id="${uid}-wrap" style="${hasPhoto?'display:block':'display:none'};position:relative">
            <img id="${uid}-img" src="${photoSrc}" style="width:42px;height:42px;border-radius:10px;object-fit:cover;cursor:pointer" onclick="varPhotoChoice('${uid}')">
          ...
          <button id="${uid}-btn" onclick="varPhotoChoice('${uid}')" style="width:42px;height:42px;border-radius:10px;background:var(--white);border:1.5px dashed var(--muted);cursor:pointer;font-size:14px;display:${hasPhoto?'none':'flex'};align-items:center;justify-content:center">📷</button>
        </div>
      </div>
    </div>`;
}
...
// ── State ──
var _lpTimer = typeof _lpTimer !== 'undefined' ? _lpTimer : null;
var _lpPid = typeof _lpPid !== 'undefined' ? _lpPid : null;
var _selectedProducts = typeof _selectedProducts !== 'undefined' ? _selectedProducts : new Set();
var _invGridMode = typeof _invGridMode !== 'undefined' ? _invGridMode : false;
var _npPhotoData = typeof _npPhotoData !== 'undefined' ? _npPhotoData : '';
var _editingProductId = typeof _editingProductId !== 'undefined' ? _editingProductId : null, _epPhotoData = '';
var _npBadge = typeof _npBadge !== 'undefined' ? _npBadge : '', _epBadge = '';
// _updatingProductId / _updatingVariantId declared in data.js

// ── Virtual Scroll ──
var _invVirtualList = typeof _invVirtualList !== 'undefined' ? _invVirtualList : [];
var _invVirtualStart = typeof _invVirtualStart !== 'undefined' ? _invVirtualStart : 0;
var INV_PAGE = typeof INV_PAGE !== 'undefined' ? INV_PAGE : 12;
var _invScrollListener = typeof _invScrollListener !== 'undefined' ? _invScrollListener : null;
...
function renderInventory(){
  ...
  document.getElementById('inv-count-sub').textContent = total+' units'+(inTransitCount>0?' · '+inTransitCount+' on the way 🚢':'');
...
function renderInvCard(p){
  ...
  const _lowThresh = parseInt(localStorage.getItem('biz_lowstock_threshold')||'10');
  const isLow = totalQty > 0 && totalQty < _lowThresh;
  ...
  const lowBadgeHtml = isLow && totalQty > 0 ? `<div style="position:absolute;top:6px;right:6px;background:#f59e0b;color:white;border-radius:6px;padding:2px 6px;font-size:9px;font-weight:800;z-index:2">Low</div>` : '';
  const zeroBadgeHtml = totalQty === 0 ? `<div style="position:absolute;top:6px;right:6px;background:var(--red);color:white;border-radius:6px;padding:2px 6px;font-size:9px;font-weight:800;z-index:2">Out</div>` : '';
...
function openInvGridView(pid){
  ...
  const html = `
    <div style="background:var(--white);border-radius:20px 20px 0 0;padding:20px 18px 36px;max-height:80dvh;overflow-y:auto">
      ...
      <!-- PRICING INFO -->
      <div style="border-radius:12px;overflow:hidden;border:1px solid var(--grey2);margin-bottom:16px">
        <div style="background:var(--grey);padding:8px 14px;font-size:10px;font-weight:700;color:var(--muted);text-transform:uppercase;letter-spacing:0.5px">💰 Pricing & Cost</div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:0">
          ${p.costSource?`<div style="padding:12px 14px;border-right:1px solid var(--grey2);border-top:1px solid var(--grey2)"><div style="font-size:9px;font-weight:700;color:var(--muted);text-transform:uppercase;letter-spacing:0.5px;margin-bottom:4px">Source Cost</div><div style="font-size:16px;font-weight:700;color:var(--ink)">$${p.costSource}</div></div>`:''}
          ${p.cost?`<div style="padding:12px 14px;border-top:1px solid var(--grey2)"><div style="font-size:9px;font-weight:700;color:var(--muted);text-transform:uppercase;letter-spacing:0.5px;margin-bottom:4px">Arrival Cost</div><div style="font-size:16px;font-weight:700;color:var(--ink)">$${p.cost}</div></div>`:''}
          ${p.priceRAPiece?`<div style="padding:12px 14px;border-right:1px solid var(--grey2);border-top:1px solid var(--grey2)"><div style="font-size:9px;font-weight:700;color:var(--muted);text-transform:uppercase;letter-spacing:0.5px;margin-bottom:4px">RA Price / pc</div><div style="font-size:16px;font-weight:700;color:var(--green)">$${p.priceRAPiece}</div></div>`:''}
          ${p.priceRADozen?`<div style="padding:12px 14px;border-top:1px solid var(--grey2)"><div style="font-size:9px;font-weight:700;color:var(--muted);text-transform:uppercase;letter-spacing:0.5px;margin-bottom:4px">RA Price / dozen</div><div style="font-size:16px;font-weight:700;color:var(--green)">$${p.priceRADozen}</div></div>`:''}
          ${p.priceFlora?`<div style="padding:12px 14px;border-right:1px solid var(--grey2);border-top:1px solid var(--grey2)"><div style="font-size:9px;font-weight:700;color:var(--muted);text-transform:uppercase;letter-spacing:0.5px;margin-bottom:4px">Flora Price</div><div style="font-size:16px;font-weight:700;color:var(--rose)">$${p.priceFlora}</div></div>`:''}
          ${(p.priceRAPiece&&p.cost)?`<div style="padding:12px 14px;border-top:1px solid var(--grey2)"><div style="font-size:9px;font-weight:700;color:var(--muted);text-transform:uppercase;letter-spacing:0.5px;margin-bottom:4px">Margin / pc</div><div style="font-size:16px;font-weight:700;color:var(--amber)">$${(p.priceRAPiece-p.cost).toFixed(2)}</div></div>`:''}
        </div>
      </div>
...
function openEditProduct(pid){
  ...
  p.variants.forEach(v=>{
    ...
    d.dataset.vid = v.id;
    d.style = 'background:var(--grey);border-radius:16px;padding:14px;margin-bottom:10px;border:1.5px solid var(--grey2)';
    d.innerHTML = _buildVariantCardHTML(uid, p.store, {
      label: v.label||'', name: v.name||'', size: v.size||'',
      colorHex: (v.colorHex && v.colorHex!=='#ede6e8') ? v.colorHex : '#ede6e8',
      hasPhoto: hasVPhoto, photoSrc: hasVPhoto ? v.photo : '',
      ra: v.ra||0, flora: v.flora||0,
      priceDozen: v.priceDozen||'', sellingPrice: v.pricePiece||'',
      barcode: v.barcode||'', isEdit: true
    });
    vc.appendChild(d);
...
function addEpVariantRow(){
  ...
  d.dataset.vid = '';
  d.style = 'background:var(--grey);border-radius:16px;padding:14px;margin-bottom:10px;border:1.5px solid var(--grey2)';
  d.innerHTML = _buildVariantCardHTML(uid, store, {isEdit:true});
  vc.appendChild(d);
}
...
function saveEditProduct(){
  ...
  p.name = name;
  p.emoji = (typeof getCatEmoji==='function'?getCatEmoji(document.getElementById('ep-cat').value||p.category||'other'):document.getElementById('ep-emoji').value)||p.emoji||'📦';
  ...
  if(!epHasVariants){
    ...
    rows.forEach((row, i)=>{
      ...
      updatedVariants.push({
        ...
        pricePiece: parseFloat(row.querySelector('.var-selling-price-inp')?.value)||0,
...
function npSetBadge(val){
  ...
  ['none','bestseller','new','clearance','lastfew'].forEach(b=>{
...
function addVariantRow(){
  ...
  d.style.cssText='background:var(--grey);border-radius:16px;padding:14px;margin-bottom:10px;transition:background 0.2s;border:1.5px solid var(--grey2)';
  d.innerHTML=_buildVariantCardHTML(uid, store, {isEdit:false});
  attachVariantLongPress(d);
...
function openAddProduct(){
  ...
  const npEmojiEl=document.getElementById('np-emoji'); if(npEmojiEl) npEmojiEl.value='📦';
...
function saveNewProduct(){
  ...
  const p = {
    id:'p-'+Date.now(),
    name, emoji:(typeof getCatEmoji==='function'?getCatEmoji(document.getElementById('np-cat').value||'other'):document.getElementById('np-emoji').value)||'📦',
