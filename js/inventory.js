...
const INV_PAGE = 12; // cards rendered at a time
...
function _renderVirtualInventory(){
  ...
  // If all fit in first batch, done
  if(_invVirtualList.length <= INV_PAGE) return;

  // Append a sentinel div that triggers loading more
  ...
  _invScrollListener = () => {
    ...
    if(rect.top <= viewH + 200){
      ...
      if(!next.length){
        sent.remove();
        if(scroller) scroller.removeEventListener('scroll', _invScrollListener);
        return;
      }
...
function saveEditProduct(){
  ...
  savePhoto(p.id, p.photo); // save to IDB_PHOTOS separately
  p.badge = _epBadge||'';
  ...
  if(!epHasVariants){
    ...
    rows.forEach((row, i)=>{
      ...
      if(vpPhoto) savePhoto(p.id+'__v__'+vid, vpPhoto); // save variant photo separately
    });
...
function deleteProduct(pid){
  ...
  appConfirm('Delete Product', `Delete "${p.name}"? This cannot be undone.`, '🗑️ Delete', ()=>{
    ...
    deleteProductPhotos(pid); // clean up photos from IDB_PHOTOS
    saveProducts();
    renderInventory(); renderCatalog(); initDashboard();
...
function saveNewProduct(){
  ...
  products.push(p);
  // Save main photo + variant photos separately (async, no need to await)
  if(p.photo) savePhoto(p.id, p.photo);
