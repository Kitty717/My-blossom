...
function bulkDeleteSelected(){
  ...
  appConfirm('Delete Products', `Delete ${n} product${n>1?'s':''}? This cannot be undone.`, `🗑️ Delete ${n}`, ()=>{
    _selectedProducts.forEach(pid=>{ const idx = products.findIndex(p=>p.id===pid); if(idx !== -1) products.splice(idx,1); deleteProductPhotos(pid); });
...
function saveEditProduct(){
  ...
  // Save main product photo to IDB_PHOTOS (photos are stripped from saveProducts())
  savePhoto(p.id, _epPhotoData||'');
  // Save variant photos to IDB_PHOTOS
  p.variants.forEach(v=>{ savePhoto(p.id+'__v__'+v.id, v.photo||''); });
  saveProducts();
  renderInventory(); if(typeof renderCatalog==='function') renderCatalog(); initDashboard();
...
function saveNewProduct(){
  ...
  const photoToSave = _npPhotoData || '';
  _npPhotoData = '';
  if(p.shipmentId){
    const linkedShip = shipments.find(s=>s.id===p.shipmentId);
    if(linkedShip && linkedShip.status!=='arrived'){
  ...
  // Save main product photo to IDB_PHOTOS (photos are stripped from saveProducts())
  if(photoToSave) savePhoto(p.id, photoToSave);
  // Save variant photos to IDB_PHOTOS
  p.variants.forEach(v=>{ if(v.photo) { savePhoto(p.id+'__v__'+v.id, v.photo); } });
  saveProducts();
  renderInventory(); if(typeof renderCatalog==='function') renderCatalog(); initDashboard();
