...
function rebuildInvTabs(){
  ...
  if(!tabs) return;
  const urgentCount = products.filter(p=>!isProductInTransit(p)&&getTotalQty(p)<(p.reorderAt||0)).length;
...
function renderInventory(){
  ...
  let list = products.filter(p=>{
    if(invFilter==='low') return p.reorderAt>0 && getTotalQty(p)<p.reorderAt;
  ...
  const lowCount = products.filter(p=>!isProductInTransit(p)&&p.reorderAt>0&&getTotalQty(p)<p.reorderAt).length;
  ...
  const reorderProds = products.filter(p=>!isProductInTransit(p)&&p.reorderAt>0&&getTotalQty(p)<p.reorderAt);
...
function renderReorderView(){
  ...
  const reorderDismissed = getReorderDismissed();
  const urgent = products.filter(p => !isProductInTransit(p) && p.reorderAt>0 && getTotalQty(p)<p.reorderAt);
  ...
  const recentlyRestocked = products.filter(p => {
    ...
    return p.restockedAt && p.restockedAt > recentCutoff && qty >= (p.reorderAt||0);
  });

  const urgentHtml = visibleUrgent.length ? visibleUrgent.map(p => `
    <div class="reorder-card" style="margin-bottom:10px">
      <div style="font-size:26px">${p.emoji}</div>
      <div class="reorder-info"><div class="reorder-name">${p.name}</div><div class="reorder-sub">Only ${getTotalQty(p)} left · reorder at ${p.reorderAt||0}</div></div>
      <div style="display:flex;flex-direction:column;gap:6px;flex-shrink:0">
        <button onclick="event.stopPropagation();createReorderTodo('${p.id}')" style="background:var(--amber);color:white;border:none;border-radius:8px;padding:5px 10px;font-size:11px;font-weight:700;cursor:pointer;font-family:'DM Sans',sans-serif;white-space:nowrap">📋 Reorder</button>
        <button onclick="snoozeReorder('${p.id}',${getTotalQty(p)})" style="background:#f59e0b;color:white;border:none;border-radius:8px;padding:5px 10px;font-size:11px;font-weight:700;cursor:pointer;font-family:'DM Sans',sans-serif;white-space:nowrap">😴 Snooze</button>
        <button onclick="dismissReorder('${p.id}',${getTotalQty(p)})" style="background:var(--red);color:white;border:none;border-radius:8px;padding:5px 10px;font-size:11px;font-weight:700;cursor:pointer;font-family:'DM Sans',sans-serif;white-space:nowrap">✕ Dismiss</button>
...
function renderInvCard(p){
  ...
  const isLow = totalQty < (p.reorderAt||0);
  ...
  return `<div class="inv-prod-card collapsed" id="ipc-${p.id}"
    ...
    <div class="inv-prod-head">
      ...
      <div class="inv-prod-info">
        ...
        ${p.category ? `<span style="display:inline-block;background:var(--grey);border-radius:20px;padding:2px 8px;font-size:10px;font-weight:600;color:var(--muted);margin-top:3px">${typeof getCatEmoji==='function'?getCatEmoji(p.category):'🏷️'} ${p.category}</span>` : ''}
        ${(p.tags||[]).length ? `<div style="display:flex;flex-wrap:wrap;gap:3px;margin-top:3px">${(p.tags||[]).map(t=>`<span style="background:var(--rose-soft);color:var(--rose);border-radius:20px;padding:1px 7px;font-size:9px;font-weight:700">#${t}</span>`).join('')}</div>` : ''}
        ${shipTag}
      </div>
...
function openEditProduct(pid){
  ...
  if(typeof initTagsField==='function') initTagsField('ep', p.tags||[]);
  ...
  document.getElementById('ep-reorder').value = p.reorderAt||0;
...
function saveEditProduct(){
  ...
  p.tags = typeof _epTags!=='undefined' ? [..._epTags] : (p.tags||[]);
  p.name = name;
  ...
  p.badge = _epBadge||'';
  // Save canChooseVariant
  const cvToggle = document.getElementById('ep-can-choose-variant');
  if(cvToggle) p.canChooseVariant = cvToggle.classList.contains('active');
  ...
  const cvEl = document.getElementById('ep-can-choose-variant');
  if(cvEl){ if(p.canChooseVariant===false) cvEl.classList.remove('active'); else cvEl.classList.add('active'); }
  ...
  // Save photo explicitly first, then save products and re-render
  const _pid = p.id;
  const _photo = _epPhotoData||'';
  _epPhotoData = '';
  if(_photo){
    savePhoto(_pid, _photo).then(()=>{
      p.photo = _photo;
      saveProducts();
      renderInventory(); if(typeof renderCatalog==='function') renderCatalog(); initDashboard();
    });
...
function openAddProduct(){
  ...
  if(typeof initTagsField==='function') initTagsField('np',[]);
  const reorderInp = document.getElementById('np-reorder');
  if(reorderInp) reorderInp.value = '0';
...
function saveNewProduct(){
  ...
  if(duplicate){
    appConfirm(
      ...
      `"${name}" already exists in your inventory. Add anyway as a separate product?`,
      '➕ Add Anyway',
      () => _doSaveNewProduct(name)
    );
    return;
  }
  _doSaveNewProduct(name);
...
function _doSaveNewProduct(name){
  ...
  const p = {
    id:'p-'+Date.now(),
    createdAt: Date.now(),
    tags: typeof _npTags!=='undefined' ? [..._npTags] : [],
    ...
    reorderAt: parseInt(document.getElementById('np-reorder')?.value,10)||0,
    badge: _npBadge || '',
    photo: _npPhotoData||'',
    barcode: document.getElementById('np-barcode').value.trim()||'',
    variants
  };
  const _newPhoto = _npPhotoData||'';
  _npPhotoData = '';
  if(_newPhoto) savePhoto(p.id, _newPhoto).then(()=>{ p.photo=_newPhoto; renderInventory(); if(typeof renderCatalog==='function') renderCatalog(); });
  if(p.shipmentId){
    const linkedShip = shipments.find(s=>s.id===p.shipmentId);
    if(linkedShip && linkedShip.status!=='arrived'){
