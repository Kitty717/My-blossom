...
function rebuildInvTabs(){
  ...
  const urgentCount = products.filter(p=>!isProductInTransit(p)&&getTotalQty(p)>0&&getTotalQty(p)<_lowThresh).length;
  // Update drawer badge silently
  ...
  const lowBadge = urgentCount > 0 ? `<span style="background:var(--red);color:white;border-radius:50px;padding:1px 6px;font-size:10px;font-weight:700;margin-left:4px">${urgentCount}</span>` : '';
  const base = `
    <div class="tab${invFilter==='all'?' active':''}" onclick="invFilter='all';ftab(this);renderInventory()">All</div>
    <div class="tab${invFilter==='low'?' active':''}" onclick="invFilter='low';ftab(this);renderInventory()">⚠️ Low${lowBadge}</div>
...
function renderInventory(){
  ...
  let list = products.filter(p=>{
    if(invFilter==='low') return getTotalQty(p)<(parseInt(localStorage.getItem('biz_lowstock_threshold')||'10'));
...
function showLowStockThresholdPicker(){
  const current = (typeof alertThresholds!=='undefined' ? alertThresholds.lowStock : null) || parseInt(localStorage.getItem('biz_lowstock_threshold')||'10');
