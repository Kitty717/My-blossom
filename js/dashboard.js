...
function initDashboard(){
  ...
  const low = visibleProds.filter(p=>p.reorderAt>0&&getTotalQty(p)<p.reorderAt).length;
  ...
  // Reorder alert
  const needReorder = products.filter(p=>!isProductInTransit(p)&&p.reorderAt>0&&getTotalQty(p)<p.reorderAt);
  ...
  const lowItems = visibleInvProds.filter(p=>p.reorderAt>0&&getTotalQty(p)<p.reorderAt).length;
...
let _saCache = null;
let _saCacheTime = 0;
const _SA_TTL = 30000; // 30 seconds

function getSmartAlerts(){
  const now = Date.now();
  if(_saCache && (now - _saCacheTime) < _SA_TTL) return _saCache;
  _saCache = _computeSmartAlerts();
  _saCacheTime = now;
  return _saCache;
}

function invalidateSACache(){ _saCache = null; }

function _computeSmartAlerts(){
  ...
  // ── INVENTORY ─────────────────────────────────────
  const zeroStock = products.filter(p => !isProductInTransit(p) && p.reorderAt>0 && getTotalQty(p)===0);
  if(zeroStock.length){
  ...
  const lowStock = products.filter(p => { if(isProductInTransit(p)) return false; if(!p.reorderAt||p.reorderAt===0) return false; const q=getTotalQty(p); return q>0 && q<p.reorderAt; });
  if(lowStock.length){
