...
const CAT_EMOJI_MAP = {
  makeup:'💄', skincare:'🧴', selfcare:'🛁', perfume:'🌹',
  bags:'👜', accessories:'💎', stationery:'✏️', giftbox:'🎁',
  decor:'🏠', other:'📦',
  // legacy mappings
  lips:'💄', face:'🧴', body:'🛁', hair:'🛁', nails:'🛁', candles:'🏠',
  'gift box':'🎁', 'bags & wear':'👜', 'self care':'🛁',
};

const CAT_KEYWORDS = {
  makeup:     ['lipstick','lip gloss','gloss','mascara','eyeliner','eyeshadow','foundation','blush','contour','beauty blender','blender','brush','palette','concealer','highlighter','bronzer','liner','lash','lashes','lip ','eye shadow','powder','setting spray','primer','makeup','cosmetic','rouge'],
  skincare:   ['serum','moisturizer','face mask','toner','sunscreen','spf','lotion','cream','cleanser','vitamin c','retinol','hyaluronic','exfoliant','micellar','eye cream','face wash','facial','peel','sheet mask','acne','skincare','skin care','essence'],
  selfcare:   ['body wash','scrub','bath bomb','body lotion','body butter','hair mask','nail ','shower','shampoo','conditioner','hair oil','scalp','exfoliating','bubble bath','bath ','self care'],
  perfume:    ['perfume','oud','mist','cologne','fragrance','spray','eau de','parfum','body spray','scent','attar'],
  bags:       ['tote','bag','hoodie','t-shirt','tshirt','cap ','hat','wallet','pouch','backpack','purse','crossbody','clothing','shirt','jacket'],
  accessories:['jewelry','ring','bracelet','necklace','earring','hair clip','scrunchie','sunglasses','accessory','accessories','charm','anklet','hairband','headband','pin ','strap'],
  stationery: ['notebook','eraser','pen ','pencil','sanrio','sticker','planner','journal','sticky','washi','marker','sketchbook','notepad','diary'],
  giftbox:    ['gift set','gift box','bundle','kit','hamper','gift '],
  decor:      ['candle','diffuser','frame','aesthetic','vase','lamp','figurine','decor','decoration','ornament','wall art'],
};

function getCatEmoji(val){ return CAT_EMOJI_MAP[(val||'').toLowerCase()] || '🏷️'; }
...
function _showCatSuggestion(suggestionElId, chipsId, hiddenId, suggestedCat){
  const el = document.getElementById(suggestionElId);
  if(!el) return;
  if(!suggestedCat){ el.style.display='none'; return; }
  const emoji = getCatEmoji(suggestedCat);
  const label = document.querySelector(`#${chipsId} [data-val="${suggestedCat}"]`)?.textContent || suggestedCat;
  el.style.display = 'block';
  el.innerHTML = `<div style="display:flex;align-items:center;gap:8px;background:var(--rose-pale);border-radius:10px;padding:8px 12px;border:1.5px solid var(--rose-light)">
    <span style="font-size:11px;color:var(--muted)">Suggested:</span>
    <button onclick="confirmCatSuggestion('${suggestedCat}','${hiddenId}','${chipsId}','${suggestionElId}')" style="background:var(--rose);color:white;border:none;border-radius:8px;padding:5px 12px;font-size:12px;font-weight:700;cursor:pointer;font-family:inherit">${label}</button>
    <button onclick="document.getElementById('${suggestionElId}').style.display='none'" style="background:none;border:none;color:var(--muted);font-size:14px;cursor:pointer;padding:4px">✕</button>
  </div>`;
}

function confirmCatSuggestion(val, hiddenId, chipsId, suggestionElId){
  const chip = document.querySelector(`#${chipsId} [data-val="${val}"]`);
  if(chip) selectCatChip(chip, hiddenId);
  document.getElementById(suggestionElId).style.display='none';
}

function npSuggestCategory(name){
  const cat = detectCategory(name);
  _showCatSuggestion('np-cat-suggestion','np-cat-chips','np-cat', cat);
}

function epSuggestCategory(name){
  const cat = detectCategory(name);
  _showCatSuggestion('ep-cat-suggestion','ep-cat-chips','ep-cat', cat);
}
...
function selectCatChip(el, hiddenId){
  ...
  // Auto-set emoji from category
  const emojiId = hiddenId==='np-cat' ? 'np-emoji' : 'ep-emoji';
  const emojiEl = document.getElementById(emojiId);
  if(emojiEl) emojiEl.value = getCatEmoji(val);
  // Hide custom input wrap if open
...
function openCustomCatChip(hiddenId, chipsId){
  ...
  if(!wrap||!inp) return;
  wrap.style.cssText = 'display:flex;margin-top:8px;gap:6px;align-items:center';
  inp.value = '';
...
function injectCustomCatChip(val){
  if(!val) return;
  const icon = getCatEmoji(val);
  ['np-cat-chips','ep-cat-chips'].forEach(chipsId=>{
    const container = document.getElementById(chipsId);
    if(!container) return;
    if(container.querySelector(`[data-val="${val}"]`)) return;
    ...
    // Long-press to delete custom category
    let lt;
    chip.addEventListener('touchstart', ()=>{ lt=setTimeout(()=>{
      appConfirm('Delete Category','Remove "'+val+'" from custom categories?','🗑️ Delete',()=>{
        const cats = getCustomCategories().filter(c=>c!==val);
        saveCustomCategories(cats);
        document.querySelectorAll(`[data-val="${val}"][data-custom="1"]`).forEach(c=>c.remove());
        showToast('Category removed');
      });
    },600); }, {passive:true});
    chip.addEventListener('touchend', ()=>clearTimeout(lt));
    chip.addEventListener('touchmove', ()=>clearTimeout(lt));
...
function loadCustomCategories(){
  ...
  // Add long-press delete to ALL cat chips (system + custom)
  setTimeout(()=>{
    document.querySelectorAll('.cat-chip').forEach(chip=>{
      if(chip.dataset.lpBound) return;
      chip.dataset.lpBound = '1';
      const val = chip.dataset.val;
      if(!val || val === '_custom') return;
      let lt;
      chip.addEventListener('touchstart', ()=>{ lt=setTimeout(()=>{
        appConfirm('Delete Category','Remove "'+val+'" category? Products using it will keep the name but the chip will be gone.','🗑️ Delete',()=>{
          // Remove from custom cats if custom
          const customs = getCustomCategories().filter(c=>c!==val);
          saveCustomCategories(customs);
          // Remove chips
          document.querySelectorAll(`.cat-chip[data-val="${val}"]`).forEach(c=>c.remove());
          showToast('Category removed');
        });
      },700); }, {passive:true});
      chip.addEventListener('touchend', ()=>clearTimeout(lt));
...
// ── Business Profile ──
const BIZ_DEFAULTS = {
  ra:    { name:'RA Warehouse',    desc:'Wholesale beauty & cosmetics', phone:'03519371', wa:'03519371', wa2:'', ig:'', address:'', hours:'', bank:'', acc:'', notes:'', logo:'' },
  flora: { name:'Flora Gift Shop', desc:'Retail gifts & accessories',   phone:'71664849',  wa:'71664849', wa2:'', ig:'', address:'', hours:'', bank:'', acc:'', notes:'', logo:'' }
};
...
function loadBizProfiles(){
  ...
  // Update WA display
  const raWaEl = document.getElementById('set-ra-wa-display');
  const floraWaEl = document.getElementById('set-flora-wa-display');
  if(raWaEl) raWaEl.textContent = BIZ_DEFAULTS.ra.wa || '03519371';
  if(floraWaEl) floraWaEl.textContent = BIZ_DEFAULTS.flora.wa || '71664849';
}
...
function _showLoginScreen(){
  const RAMEZ_MESSAGES = [
    "Good morning habibi 💛 Your empire awaits 👑",
    "Hey Ramez! Aya built all of this for you both 🌸",
    "Welcome back habibi 💕 Let's check on the business",
    "Yalla Ramez, let's see how we're doing today 📊",
    "Hi habibi! Everything is ready for you 🌟",
    "Welcome back! Aya says hi 💛",
    "Good to see you Ramez 🌹 The business is yours too",
  ];
  const dayMsg = RAMEZ_MESSAGES[new Date().getDate() % RAMEZ_MESSAGES.length];

  const ov = document.createElement('div');
  ov.id = 'login-screen';
  ov.style.cssText = 'position:fixed;inset:0;z-index:99999;background:linear-gradient(160deg,#1a0a10 0%,#2d1020 50%,#1a0a10 100%);display:flex;flex-direction:column;align-items:center;justify-content:center;padding:32px 24px;font-family:\'DM Sans\',sans-serif';
  ov.innerHTML = `
    <div style="position:absolute;width:300px;height:300px;background:radial-gradient(circle,rgba(201,144,10,0.2) 0%,transparent 70%);top:-80px;left:-60px;border-radius:50%;pointer-events:none"></div>
    <div style="position:absolute;width:240px;height:240px;background:radial-gradient(circle,rgba(192,83,106,0.25) 0%,transparent 70%);bottom:-60px;right:-40px;border-radius:50%;pointer-events:none"></div>
    <div style="font-family:'Playfair Display',serif;font-size:36px;color:white;margin-bottom:4px;position:relative;z-index:1">Blossom</div>
    <div style="font-size:10px;font-weight:700;letter-spacing:4px;text-transform:uppercase;color:rgba(201,144,10,0.8);margin-bottom:48px;position:relative;z-index:1">Business</div>
    <div style="display:flex;flex-direction:column;gap:14px;width:100%;max-width:320px;position:relative;z-index:1">
      <button onclick="loginAs('aya')" style="background:rgba(255,255,255,0.08);border:1.5px solid rgba(255,255,255,0.15);border-radius:20px;padding:20px;cursor:pointer;font-family:inherit;text-align:left;transition:all 0.25s;backdrop-filter:blur(8px)" onmouseenter="this.style.background='rgba(201,144,10,0.15)';this.style.borderColor='rgba(201,144,10,0.4)'" onmouseleave="this.style.background='rgba(255,255,255,0.08)';this.style.borderColor='rgba(255,255,255,0.15)'">
        <div style="font-size:28px;margin-bottom:8px">🌸</div>
        <div style="font-size:18px;font-weight:700;color:white;margin-bottom:3px">Aya</div>
        <div style="font-size:12px;color:rgba(255,255,255,0.5)">RA Jemle & Flora Gift Shop</div>
      </button>
      <button onclick="loginAs('ramez')" style="background:rgba(255,255,255,0.08);border:1.5px solid rgba(255,255,255,0.15);border-radius:20px;padding:20px;cursor:pointer;font-family:inherit;text-align:left;transition:all 0.25s;backdrop-filter:blur(8px)" onmouseenter="this.style.background='rgba(201,144,10,0.15)';this.style.borderColor='rgba(201,144,10,0.4)'" onmouseleave="this.style.background='rgba(255,255,255,0.08)';this.style.borderColor='rgba(255,255,255,0.15)'">
        <div style="font-size:28px;margin-bottom:8px">👑</div>
        <div style="font-size:18px;font-weight:700;color:white;margin-bottom:3px">Ramez</div>
        <div style="font-size:12px;color:rgba(255,255,255,0.5)">Full access · Partner</div>
      </button>
    </div>
  `;
...
async function loginAs(user){
  localStorage.setItem('blossom_active_user', user);
  window.blossomActiveUser = user;
  const screen = document.getElementById('login-screen');

  if(user === 'ramez'){
    // Show welcome message for Ramez
    const RAMEZ_MESSAGES = [
      "Good morning habibi 💛 Your empire awaits 👑",
      "Hey Ramez! Aya built all of this for you both 🌸",
      "Welcome back habibi 💕 Let's check on the business",
    ...
    const msg = RAMEZ_MESSAGES[new Date().getDate() % RAMEZ_MESSAGES.length];
    screen.innerHTML = `
      <div style="text-align:center;padding:40px 24px;font-family:'DM Sans',sans-serif">
        <div style="font-size:64px;margin-bottom:20px;animation:bounceIn 0.6s cubic-bezier(0.34,1.7,0.64,1)">👑</div>
        <div style="font-family:'Playfair Display',serif;font-size:28px;color:white;margin-bottom:12px">Welcome, Ramez!</div>
        <div style="font-size:15px;color:rgba(240,208,128,0.9);line-height:1.6;max-width:280px;margin:0 auto">${msg}</div>
        <style>@keyframes bounceIn{from{transform:scale(0) rotate(-20deg);opacity:0}to{transform:scale(1) rotate(0);opacity:1}}</style>
      </div>`;
  ...
  // Show onboarding for Ramez on first time
  if(user === 'ramez' && !localStorage.getItem('ramez_onboarded')){
    setTimeout(()=>_showRamezOnboarding(), 800);
  }
}

function _applyUserTheme(user){
  // Both users see same app — just store active user
  window.blossomActiveUser = user;
}
...
async function _bootApp(){
  ...
  setTimeout(()=>{ syncRamezDashboard(); }, 6000); // sync CEO dashboard
...
function _showRamezOnboarding(){
  const steps = [
    { icon:'📊', title:'Dashboard', desc:'Your business overview at a glance — sales, stock, customers, everything.' },
    { icon:'📦', title:'Products', desc:'All inventory for RA Jemle and Flora. Tap a product to see stock and details.' },
    { icon:'🧾', title:'Invoices', desc:'Every sale is here. Track what\'s paid, shipped, or pending.' },
    { icon:'👥', title:'Customers', desc:'All your customers with their order history and debt status.' },
    { icon:'💰', title:'Finance', desc:'Revenue, expenses, profit per shipment. Your full financial picture.' },
    { icon:'🛍️', title:'Shop Manager', desc:'Control the customer shop — orders, visibility, settings.' },
  ];
  let current = 0;

  const ov = document.createElement('div');
  ov.id = 'ramez-onboarding';
  ov.style.cssText = 'position:fixed;inset:0;z-index:99998;background:rgba(26,10,16,0.85);backdrop-filter:blur(12px);display:flex;align-items:flex-end;justify-content:center;font-family:\'DM Sans\',sans-serif';

  function render(){
    const s = steps[current];
    const isLast = current === steps.length - 1;
    ov.innerHTML = `
      <div style="background:linear-gradient(160deg,#1a0a10,#2d1020);border-radius:24px 24px 0 0;width:100%;max-width:480px;padding:28px 24px 40px">
        <div style="display:flex;gap:6px;margin-bottom:20px">
          ${steps.map((_,i)=>`<div style="flex:1;height:3px;border-radius:2px;background:${i<=current?'#c9900a':'rgba(255,255,255,0.15)'}"></div>`).join('')}
        </div>
        <div style="font-size:48px;margin-bottom:14px">${s.icon}</div>
        <div style="font-family:'Playfair Display',serif;font-size:24px;color:white;margin-bottom:8px">${s.title}</div>
        <div style="font-size:14px;color:rgba(255,255,255,0.6);line-height:1.6;margin-bottom:28px">${s.desc}</div>
        <div style="display:flex;gap:10px">
          <button onclick="document.getElementById('ramez-onboarding').remove();localStorage.setItem('ramez_onboarded','1')" style="flex:1;padding:13px;background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.15);border-radius:14px;color:rgba(255,255,255,0.5);font-family:inherit;font-size:13px;font-weight:600;cursor:pointer">Skip tour</button>
          <button onclick="ramezOnboardNext()" style="flex:2;padding:13px;background:linear-gradient(135deg,#c9900a,#e0a820);border:none;border-radius:14px;color:#1a0a10;font-family:inherit;font-size:14px;font-weight:800;cursor:pointer">${isLast?'Let\'s go! 🚀':'Next →'}</button>
        </div>
      </div>`;
  }

  window.ramezOnboardNext = ()=>{
    if(current < steps.length - 1){ current++; render(); }
    else { ov.remove(); localStorage.setItem('ramez_onboarded','1'); }
  };
...
const VCOLOR_CATS = [
  { label:'⭐ Custom', colors:[], isCustom:true },
...
function _loadCustomColors(){
  try { return JSON.parse(localStorage.getItem('vcolor_custom')||'[]'); } catch(e){ return []; }
}
function _saveCustomColor(hex){
  const saved = _loadCustomColors();
  const upper = hex.toUpperCase();
  if(saved.includes(upper)) return; // already saved
  saved.unshift(upper); // newest first
  if(saved.length > 24) saved.length = 24; // max 24 custom colors
  localStorage.setItem('vcolor_custom', JSON.stringify(saved));
  _vcolorBuilt = false; // force rebuild to show new color
  showToast('Color saved to Custom ⭐');
}
function _removeCustomColor(hex){
  const saved = _loadCustomColors().filter(c => c !== hex.toUpperCase());
  localStorage.setItem('vcolor_custom', JSON.stringify(saved));
  _vcolorBuilt = false;
  showToast('Color removed');
  _buildVColorUI();
  _vcolorSwitchCat(0, false);
}
...
function openVColorPicker(btn){
  ...
  // Fallback: find hidden input sibling
  if(!_vcolorTarget && btn.previousElementSibling?.classList.contains('variant-color-input')){
    _vcolorTarget = btn.previousElementSibling;
  }
  if(!_vcolorTarget){
    const parent = btn.parentElement;
    if(parent) _vcolorTarget = parent.querySelector('.variant-color-input') || parent.querySelector('input[type="hidden"]');
  }
...
function _buildVColorUI(){
  ...
  VCOLOR_CATS.forEach((cat, catIdx)=>{
    const page = document.createElement('div');
    page.dataset.catIdx = catIdx;
    if(cat.isCustom){
      // Custom tab — rendered dynamically
      page.style.cssText = `flex-shrink:0;width:100%;padding:12px 18px 4px;`;
      const customColors = _loadCustomColors();
      if(!customColors.length){
        page.innerHTML = `<div style="color:var(--muted);font-size:13px;text-align:center;padding:20px 0">
          No saved colors yet.<br>
          <span style="font-size:11px">Pick a color from photo or type a hex,<br>then tap ⭐ Save to add it here.</span>
        </div>`;
      } else {
        page.style.cssText += `display:grid;grid-template-columns:repeat(8,1fr);gap:8px;align-content:start`;
        customColors.forEach(hex=>{
          const c = document.createElement('div');
          c.style.cssText = `aspect-ratio:1;border-radius:50%;background:${hex};cursor:pointer;border:2px solid rgba(0,0,0,0.08);transition:transform 0.12s,border 0.12s;position:relative`;
          ...
          let lt; c.addEventListener('touchstart',()=>{ lt=setTimeout(()=>{ if(confirm('Remove this color?')) _removeCustomColor(hex); },600); },{passive:true});
...
function _vcolorSwitchCat(i, scroll){
  _vcolorActiveCat = i;
  // If switching to Custom tab, rebuild to show latest saved colors
  if(VCOLOR_CATS[i]?.isCustom && !_vcolorBuilt){
    _buildVColorUI(); _vcolorBuilt = true;
  } else if(VCOLOR_CATS[i]?.isCustom){
    // Re-render custom page in place
    const slider = document.getElementById('vcolor-slider');
    if(slider){
      const customPage = slider.children[0];
      if(customPage){
        const customColors = _loadCustomColors();
        customPage.innerHTML = '';
        customPage.style.cssText = `flex-shrink:0;width:100%;padding:12px 18px 4px;`;
        if(!customColors.length){
          customPage.innerHTML = `<div style="color:var(--muted);font-size:13px;text-align:center;padding:20px 0">No saved colors yet.<br><span style="font-size:11px">Pick a color from photo or type a hex,<br>then tap ⭐ Save to add it here.</span></div>`;
        } else {
          customPage.style.cssText += `display:grid;grid-template-columns:repeat(8,1fr);gap:8px;align-content:start`;
          customColors.forEach(hex=>{
            const c = document.createElement('div');
            c.style.cssText = `aspect-ratio:1;border-radius:50%;background:${hex};cursor:pointer;border:2px solid rgba(0,0,0,0.08);transition:transform 0.12s`;
            c.onclick = ()=>{ _vcolorSelected=hex; updateVColorPreview(); highlightVColorGrid(hex);
              document.getElementById('vcolor-custom-input').value=hex.replace('#','');
              document.getElementById('vcolor-custom-swatch').style.background=hex; };
            c.dataset.hex = hex;
            let lt; c.addEventListener('touchstart',()=>{ lt=setTimeout(()=>{ if(confirm('Remove?')) _removeCustomColor(hex); },600); },{passive:true});
            c.addEventListener('touchend',()=>clearTimeout(lt));
            customPage.appendChild(c);
          });
        }
      }
    }
...
function saveCurrentVColorToCustom(){
  if(!_vcolorSelected || _vcolorSelected==='#ede6e8') { showToast('Pick a color first','err'); return; }
  _saveCustomColor(_vcolorSelected);
}
...
function extractColorFromPhoto(input){
  ...
  reader.onload = function(e){
    ...
    img.onload = function(){
      ...
      document.getElementById('vcolor-custom-input').value = hex.replace('#','');
      document.getElementById('vcolor-custom-swatch').style.background = hex;
      _saveCustomColor(hex); // auto-save detected color to Custom tab
      showToast('Color detected & saved to Custom ⭐','ok');
    };
  ...
  try {
    ...
    el.innerHTML = `
      <div style="display:flex;align-items:center;justify-content:space-between;margin:14px 0 8px">
        <div style="font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:1px;color:var(--muted);display:flex;align-items:center;gap:8px">🕐 Recent<span style="display:block;width:40px;height:1px;background:var(--grey2)"></span></div>
        <button onclick="clearAiHistory()" style="background:none;border:none;font-size:11px;color:var(--muted);cursor:pointer;font-family:inherit;padding:2px 6px">Clear</button>
      </div>` +
      hist.map((h,i)=>`<div style="background:var(--grey);border-radius:10px;padding:10px 12px;margin-bottom:6px;cursor:pointer" onclick="reuseAiMsg(${i})">
        <div style="font-size:11px;color:var(--muted);margin-bottom:4px">${new Date(h.ts).toLocaleDateString('en',{month:'short',day:'numeric',hour:'2-digit',minute:'2-digit'})}</div>
        <div style="font-size:12px;color:var(--ink);white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${h.text.slice(0,80)}...</div>
      </div>`).join('');
  } catch(e){}
}

function clearAiHistory(){
  localStorage.removeItem('biz_ai_history');
  renderAiHistory();
  showToast('History cleared');
}

function reuseAiMsg(idx){
  try {
    const hist = JSON.parse(localStorage.getItem('biz_ai_history')||'[]');
    const h = hist[idx];
    if(!h) return;
...
// ═══════════════════════════════════════════════════
// RAMEZ CEO DASHBOARD SYNC
// Pushes full data snapshot to Firebase silently
// ═══════════════════════════════════════════════════
async function syncRamezDashboard(){
  try {
    const FB = 'https://ra-shop-3e01d-default-rtdb.firebaseio.com';
    const now = new Date();
    const todayStr = now.toISOString().split('T')[0];
    const monthStr = todayStr.slice(0,7);

    // ── Finance ──
    const paidInvs = (invoices||[]).filter(i=>i.status==='paid'||i.status==='partial');
    const totalRevenue = paidInvs.reduce((s,i)=>s+(i.status==='paid'?i.total:i.paidAmt||0),0);
    const todayRevenue = paidInvs.filter(i=>i.date===todayStr).reduce((s,i)=>s+(i.status==='paid'?i.total:i.paidAmt||0),0);
    const monthRevenue = paidInvs.filter(i=>(i.date||'').startsWith(monthStr)).reduce((s,i)=>s+(i.status==='paid'?i.total:i.paidAmt||0),0);
    const totalDebt = (customers||[]).reduce((s,c)=>s+(c.debt||0),0);
    const totalExpenses = (expenses||[]).filter(e=>(e.date||'').startsWith(monthStr)).reduce((s,e)=>s+(e.amount||0),0);
    const monthProfit = monthRevenue - totalExpenses;
    const totalLosses = (losses||[]).filter(l=>(l.date||'').startsWith(monthStr)).reduce((s,l)=>s+(l.amount||0),0);
    const adSpend = (adCampaigns||[]).reduce((s,a)=>s+(a.spent||0),0);
    const adRevenue = (adCampaigns||[]).reduce((s,a)=>s+(a.revenue||0),0);
    ...
    for(let i=5;i>=0;i--){
      ...
      const rev=(invoices||[]).filter(inv=>inv.status==='paid'&&(inv.date||'').startsWith(k)).reduce((s,inv)=>s+(inv.total||0),0);
      const raRev=(invoices||[]).filter(inv=>inv.status==='paid'&&inv.store==='ra'&&(inv.date||'').startsWith(k)).reduce((s,inv)=>s+(inv.total||0),0);
      const flRev=(invoices||[]).filter(inv=>inv.status==='paid'&&inv.store==='flora'&&(inv.date||'').startsWith(k)).reduce((s,inv)=>s+(inv.total||0),0);
      trend.push({month:k,lbl,rev,raRev,flRev});
    }

    // Today counts
    const todayInvs = (invoices||[]).filter(i=>i.date===todayStr).length;
    const todayCusts = (customers||[]).filter(c=>c.createdAt&&c.createdAt.startsWith(todayStr)).length;

    // Health score (0-100)
    const margin = monthRevenue>0?monthProfit/monthRevenue:0;
    const debtRatio = totalRevenue>0?totalDebt/totalRevenue:1;
    const arrivedShips = (shipments||[]).filter(s=>s.status==='arrived');
    const recoveredShips = arrivedShips.filter(s=>{
      const collected=s.manualCollected||0; return collected>=(s.cost||0)&&s.cost>0;
    }).length;
    const recoveryRate = arrivedShips.length>0?recoveredShips/arrivedShips.length:1;
    const healthScore = Math.round(Math.max(0,Math.min(100,
      (margin*40) + ((1-Math.min(1,debtRatio))*30) + (recoveryRate*30)
    )));
    const healthLabel = healthScore>=80?'Excellent 🌟':healthScore>=65?'Strong 💪':healthScore>=50?'Steady 📈':healthScore>=35?'Needs Attention ⚠️':'Critical 🚨';

    // ── Full invoices with items ──
    const invSnapshot = (invoices||[]).map(i=>({
      id:i.id, num:i.num, customer:i.customer, total:i.total,
      status:i.status, date:i.date, store:i.store||'ra',
      paidAmt:i.paidAmt||0,
      items:(i.items||[]).map(it=>({
        productId:it.productId||'', name:it.name||'',
        qty:it.qty||1, price:it.price||0, total:it.total||0,
        variantLabel:it.variantLabel||''
      })),
      notes:i.notes||''
    }));

    // ── Customers with notes + debt aging ──
    const custSnapshot = (customers||[]).filter(c=>!c.blacklisted).map(c=>{
      const custInvs = (invoices||[]).filter(i=>(i.customer||'')===(c.name||'')&&(i.status==='unpaid'||i.status==='shipped'));
      let oldestDebt = null;
      custInvs.forEach(i=>{ if(!oldestDebt||i.date<oldestDebt) oldestDebt=i.date; });
      const debtDays = oldestDebt?Math.floor((now-new Date(oldestDebt+'T12:00:00'))/86400000):0;
      return {
        id:c.id, name:c.name, debt:c.debt||0, phone:c.phone||'',
        vip:c.vip||false, fromShop:c.fromShop||false,
        lastOrderDate:c.lastOrderDate||'', city:c.city||'',
        notes:c.notes||'', debtDays
      };
    });

    // ── Products ──
    const prodSnapshot = (products||[]).map(p=>({
      ...
      variants:(p.variants||[]).map(v=>({
        ...
        ra:v.ra||0, flora:v.flora||0, colorHex:v.colorHex||'', photo:v.photo||''
      }))
    }));

    // ── Shipments with full details ──
    const shipSnapshot = (shipments||[]).map(s=>({
      id:s.id, name:s.name, supplier:s.supplier||'',
      status:s.status, cost:s.cost||0,
      eta:s.eta||'', orderDate:s.orderDate||'', arrivedDate:s.arrivedDate||'',
      num:s.num||'', notes:s.notes||'',
      manualCollected:s.manualCollected||0,
      shortLoss:s.shortLoss||0,
      products:(products||[]).filter(p=>p.shipmentId===s.id).map(p=>({
        id:p.id, name:p.name, emoji:p.emoji||'📦', photo:p.photo||'',
        totalStock:(p.variants||[]).reduce((t,v)=>t+(v.ra||0)+(v.flora||0),0)
      }))
    }));

    // ── Flora orders ──
    const floraSnap = (floraOrders||[]).slice(-100).map(o=>({
      id:o.id, num:o.num, customer:o.customer||'',
      total:o.total||0, status:o.status, date:o.date||'',
      items:(o.items||[]).map(it=>({name:it.name||'',qty:it.qty||1,price:it.price||0}))
    }));

    // ── Expenses all time ──
    const expSnap = (expenses||[]).map(e=>({
      id:e.id, name:e.name||'', amount:e.amount||0,
      category:e.category||'', date:e.date||'', store:e.store||''
    }));

    // ── Losses ──
    const lossSnap = (losses||[]).map(l=>({
      id:l.id, name:l.name||l.reason||'', amount:l.amount||0,
      category:l.category||'', date:l.date||'', store:l.store||'',
      qty:l.qty||0, notes:l.notes||''
    }));

    // ── Ad Campaigns ──
    const adSnap = (adCampaigns||[]).map(a=>({
      id:a.id, name:a.name||'', platform:a.platform||'',
      spent:a.spent||0, revenue:a.revenue||0, unitsSold:a.unitsSold||0,
      startDate:a.startDate||'', endDate:a.endDate||'',
      roi:a.spent>0?Math.round(((a.revenue-a.spent)/a.spent)*100):0
    }));

    // ── Bundles ──
    const bundleSnap = (bundles||[]).map(b=>({
      id:b.id, name:b.name||'Bundle', cost:b.cost||0,
      pkgCost:b.pkgCost||0, sellPrice:b.sellPrice||0,
      waMessage:b.waMessage||'',
      items:(b.items||[]).map(it=>{
        const prod=(products||[]).find(p=>p.id===it.pid)||{};
        const vari=(prod.variants||[]).find(v=>v.id===it.vid)||{};
        return {
          name:prod.name||it.name||'', qty:it.qty||1,
          variantLabel:vari.label||vari.name||'',
          photo:vari.photo||prod.photo||'',
          colorHex:vari.colorHex||''
        };
      })
    }));

    // ── Collections ──
    const collSnap = (collections||[]).map(c=>({
      id:c.id, name:c.name||'', emoji:c.emoji||'🗂️',
      description:c.description||'',
      productIds:c.productIds||[],
      products:(c.productIds||[]).slice(0,10).map(pid=>{
        const p=(products||[]).find(x=>x.id===pid)||{};
        return {id:p.id||pid, name:p.name||'', photo:p.photo||'', emoji:p.emoji||'📦',
          totalStock:(p.variants||[]).reduce((s,v)=>s+(v.ra||0)+(v.flora||0),0)};
      })
    }));

    // ── Inventory counts history ──
    const scHist = window._scHistoryCache||[];
    const countSnap = scHist.slice(-20).map(h=>({
      id:h.id||h.date, date:h.date||'', label:h.label||h.date||'',
      store:h.store||'all', totalCounted:h.totalCounted||0,
      items:(h.items||[]).map(it=>({
        productId:it.productId||'', productName:it.productName||it.name||'',
        variantId:it.variantId||'', variantLabel:it.variantLabel||'',
        counted:it.counted||it.count||0, expected:it.expected||0,
        diff:(it.counted||it.count||0)-(it.expected||0)
      }))
    }));

    // ── Top debtors ──
    const topDebtors = [...(customers||[])]
      .filter(c=>c.debt>0&&!c.blacklisted)
      .sort((a,b)=>(b.debt||0)-(a.debt||0))
      .slice(0,10)
      .map(c=>{
        const custInvs=(invoices||[]).filter(i=>(i.customer||'')===(c.name||'')&&(i.status==='unpaid'||i.status==='shipped'));
        let oldest=null;
        custInvs.forEach(i=>{if(!oldest||i.date<oldest)oldest=i.date;});
        const days=oldest?Math.floor((now-new Date(oldest+'T12:00:00'))/86400000):0;
        return {name:c.name,debt:c.debt,phone:c.phone||'',debtDays:days};
      });
    ...
    const monthlyReport = {
      ...
      profit: monthProfit,
      losses: totalLosses,
      newCustomers: (customers||[]).filter(c=>c.createdAt&&c.createdAt.startsWith(monthStr)).length,
      paidInvoices: (invoices||[]).filter(i=>i.status==='paid'&&(i.date||'').startsWith(monthStr)).length,
      unpaidInvoices: (invoices||[]).filter(i=>(i.status==='unpaid'||i.status==='shipped')&&(i.date||'').startsWith(monthStr)).length,
      topCustomer: [...(customers||[])].sort((a,b)=>(b.debt||0)-(a.debt||0))[0]?.name||'',
    ...
    const snapshot = {
      ...
      updatedAtStr: now.toLocaleString('en',{weekday:'short',month:'short',day:'numeric',hour:'2-digit',minute:'2-digit'}),
      finance: {
        todayRevenue, monthRevenue, totalRevenue,
        totalDebt, totalExpenses, monthProfit, totalLosses,
        adSpend, adRevenue, trend, healthScore, healthLabel
      },
      daily: { todayInvs, todayRevenue, todayCusts },
      invoices: invSnapshot,
      customers: custSnapshot,
      products: prodSnapshot,
      shipments: shipSnapshot,
      floraOrders: floraSnap,
      expenses: expSnap,
      losses: lossSnap,
      adCampaigns: adSnap,
      bundles: bundleSnap,
      collections: collSnap,
      inventoryCounts: countSnap,
      topDebtors,
      monthlyReport,
      counts: {
        totalInvoices: (invoices||[]).length,
        unpaidInvoices: (invoices||[]).filter(i=>i.status==='unpaid'||i.status==='shipped').length,
        totalCustomers: (customers||[]).filter(c=>!c.blacklisted).length,
        vipCustomers: (customers||[]).filter(c=>c.vip).length,
        totalProducts: (products||[]).length,
        lowStockProducts: (products||[]).filter(p=>(p.variants||[]).reduce((s,v)=>s+(v.ra||0)+(v.flora||0),0)<5).length,
        outOfStockProducts: (products||[]).filter(p=>(p.variants||[]).reduce((s,v)=>s+(v.ra||0)+(v.flora||0),0)===0).length,
        shipmentsInTransit: (shipments||[]).filter(s=>s.status==='onway'||s.status==='ordered').length,
    ...
    await fetch(`${FB}/ceo_dashboard.json`,{
      method:'PUT',
      headers:{'Content-Type':'application/json'},
      body: JSON.stringify(snapshot)
    });
    console.log('✅ CEO dashboard synced - full snapshot');
  } catch(e){
    console.log('CEO sync failed (non-critical):', e.message);
  }
}
