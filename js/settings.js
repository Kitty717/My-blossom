...
async function syncRamezDashboard(){
  try {
    ...
    const shipmentCosts = (shipments||[]).filter(s=>s.status==='arrived').reduce((s,sh)=>s+(sh.cost||0),0);
    ...
    const invSnapshot = (invoices||[]).map(i=>({
      ...
      status:i.status, date:i.date, store:i.store,
      paidAmt:i.paidAmt||0, items:(i.items||[]).length
    }));
    ...
    const shipSnapshot = (shipments||[]).map(s=>({
      ...
      status:s.status, cost:s.cost||0, eta:s.eta||'',
      manualCollected:s.manualCollected||0,
      prodCount:(products||[]).filter(p=>p.shipmentId===s.id).length
    }));

    // ── Flora orders ──
    const floraSnap = (floraOrders||[]).slice(-50).map(o=>({
      id:o.id, num:o.num, customer:o.customer||'',
      total:o.total||0, status:o.status, date:o.date||''
    }));

    // ── Expenses ──
    const expSnap = (expenses||[]).filter(e=>(e.date||'').startsWith(monthStr)).map(e=>({
    ...
    const snapshot = {
      ...
      finance: {
        ...
        totalDebt, totalExpenses, monthProfit, shipmentCosts
      },
    ...
    console.log('✅ CEO dashboard synced');
  } catch(e){
