...
function injectCustomCatChip(val){
  if(!val) return;
  // Get icon from built-in categories if it matches, else use tag emoji
  const CAT_ICONS = {lips:'💄',face:'✨',body:'🧴',hair:'💇',nails:'💅',candles:'🕯️',giftbox:'🎁',decor:'🏠',accessories:'🧣',other:'📦','bags & wear':'👜🧣','bags&wear':'👜','stationery':'✏️','makeup':'💄'};
  const icon = CAT_ICONS[val.toLowerCase()] || '🏷️';
