...
function getHiddenCategories(){ try{ return JSON.parse(localStorage.getItem('hiddenCategories')||'[]'); }catch(e){ return []; } }
function saveHiddenCategories(arr){ localStorage.setItem('hiddenCategories', JSON.stringify(arr)); }

function hideBuiltInCategory(val){
  appConfirm('Hide Category','Hide "'+val+'" from the category list? You can restore it from Settings.',
    '🙈 Hide', ()=>{
      const hidden = getHiddenCategories();
      if(!hidden.includes(val)){ hidden.push(val); saveHiddenCategories(hidden); }
      document.querySelectorAll(`[data-val="${val}"][data-builtin="1"]`).forEach(c=>c.remove());
      showToast('Category hidden — restore in Settings');
    });
}

function restoreAllCategories(){
  saveHiddenCategories([]);
  // Re-inject all built-in chips
  ['np-cat-chips','ep-cat-chips'].forEach(chipsId=>{
    ...
    const BUILTIN = [
      {val:'makeup',label:'💄 Makeup'},{val:'skincare',label:'🧴 Skincare'},
      {val:'selfcare',label:'🛁 Self Care'},{val:'perfume',label:'🌹 Perfume'},
      {val:'bags',label:'👜 Bags & Wear'},{val:'accessories',label:'💎 Accessories'},
      {val:'stationery',label:'✏️ Stationery'},{val:'giftbox',label:'🎁 Gift Box'},
      {val:'decor',label:'🏠 Decor'},{val:'other',label:'📦 Other'}
    ];
    BUILTIN.forEach(({val,label})=>{
      if(container.querySelector(`[data-val="${val}"]`)) return;
      const chip = document.createElement('div');
      chip.className = 'cat-chip';
      chip.dataset.val = val;
      chip.dataset.builtin = '1';
      chip.textContent = label;
      chip.onclick = function(){ selectCatChip(chip, hiddenId); };
      _addLongPressHide(chip, val);
  ...
  showToast('All categories restored ✅');
}

function _addLongPressHide(chip, val){
  let lt;
  chip.addEventListener('touchstart', ()=>{ lt=setTimeout(()=>{ hideBuiltInCategory(val); },600); }, {passive:true});
...
function detectCategory(name){
  ...
  // Check custom categories — if product name contains the category name
...
function confirmCustomCat(hiddenId, chipsId, inp){
  ...
  // Also add the category name as a keyword for future auto-detection
  if(!CAT_KEYWORDS[val.toLowerCase()]){
    CAT_KEYWORDS[val.toLowerCase()] = [val.toLowerCase()];
  }
...
<div class="wrap">
  ...
  // Use Blob URL to avoid popup blocker on Android
  try {
    ...
    if(!w){ showToast('Allow popups in browser settings','err'); return; }
...
function handleTagKey(e, prefix){
  ...
  if((e.key==='Enter'||e.key===','||e.key===' ') && val){
    ...
    addTag(val, prefix);
    inp.value='';
    document.getElementById(prefix+'-tag-suggestions').style.display='none';
  } else if(e.key==='Backspace' && !inp.value){
    const arr = prefix==='np'?_npTags:_epTags;
    if(arr.length){ arr.pop(); renderTags(prefix); }
  }
}
...
function addTag(tag, prefix){
  ...
  if(!tag) return;
  const arr = prefix==='np'?_npTags:_epTags;
  if(!arr.includes(tag)){ arr.push(tag); renderTags(prefix); }
}

function removeTag(tag, prefix){
  if(prefix==='np') _npTags = _npTags.filter(t=>t!==tag);
  else _epTags = _epTags.filter(t=>t!==tag);
  renderTags(prefix);
}

function renderTags(prefix){
  const list = document.getElementById(prefix+'-tags-list');
  if(!list) return;
  const arr = prefix==='np'?_npTags:_epTags;
  list.innerHTML = arr.map(t=>`<span style="display:inline-flex;align-items:center;gap:4px;background:var(--rose);color:white;border-radius:20px;padding:3px 10px;font-size:11px;font-weight:700">${t}<button onclick="removeTag('${t}','${prefix}')" style="background:none;border:none;color:white;cursor:pointer;font-size:12px;padding:0;line-height:1;opacity:0.8">×</button></span>`).join('');
}

function showTagSuggestions(val, prefix){
  const el = document.getElementById(prefix+'-tag-suggestions');
  if(!el) return;
  if(!val){ el.style.display='none'; return; }
  const all = getAllUsedTags();
  const matches = all.filter(t=>t.includes(val.toLowerCase()) && !(prefix==='np'?_npTags:_epTags).includes(t));
  if(!matches.length){ el.style.display='none'; return; }
  el.style.display='flex';
  el.innerHTML = matches.slice(0,8).map(t=>`<span onclick="addTag('${t}','${prefix}');document.getElementById('${prefix}-tag-input').value='';document.getElementById('${prefix}-tag-suggestions').style.display='none'" style="background:var(--grey);border-radius:20px;padding:3px 10px;font-size:11px;cursor:pointer;font-weight:600;color:var(--ink)">${t}</span>`).join('');
}

function initTagsField(prefix, tags){
  if(prefix==='np') _npTags = tags ? [...tags] : [];
  else _epTags = tags ? [...tags] : [];
  renderTags(prefix);
  const inp = document.getElementById(prefix+'-tag-input');
  if(inp) inp.value='';
}
