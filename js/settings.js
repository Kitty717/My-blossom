...
function getAllUsedTags(){
  const tags = new Set();
  (typeof products!=='undefined'?products:[]).forEach(p => (p.tags||[]).forEach(t => tags.add(t)));
  return [...tags].sort();
}
function handleTagKey(e, prefix){
  ...
  if((e.key==='Enter'||e.key===','||e.key===' ') && val){
    ...
    addTag(val, prefix);
    inp.value='';
    const sg = document.getElementById(prefix+'-tag-suggestions');
...
function showTagSuggestions(val, prefix){
  ...
  const arr = prefix==='np'?_npTags:_epTags;
  const matches = getAllUsedTags().filter(t=>t.includes(val.toLowerCase()) && !arr.includes(t));
...
function hideBuiltInCategory(val){
  if(typeof appConfirm==='function'){
    appConfirm('Hide Category','Hide "'+val+'" from the list? Restore in Settings.','🙈 Hide',()=>{
      const h=getHiddenCategories(); if(!h.includes(val)){h.push(val);saveHiddenCategories(h);}
      document.querySelectorAll('[data-val="'+val+'"][data-builtin="1"]').forEach(c=>c.remove());
      if(typeof showToast==='function') showToast('Category hidden — restore in Settings');
    });
  }
}
