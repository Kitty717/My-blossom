...
function safeOpenAddProduct(){
  if(typeof openAddProduct === 'function') openAddProduct();
}

function restoreAllCategories(){
  if(!confirm('Reset all custom categories to defaults?')) return;
  saveCustomCategories([]);
  showToast('Categories restored ✓','ok');
}

function saveCurrentVColorToCustom(){
  if(!_vcolorSelected) return;
  const key = 'blossom_custom_vcolors';
  let saved = [];
  try { saved = JSON.parse(localStorage.getItem(key)||'[]'); } catch(e){}
  if(!saved.includes(_vcolorSelected)){
    saved.unshift(_vcolorSelected);
    if(saved.length > 16) saved = saved.slice(0,16);
    localStorage.setItem(key, JSON.stringify(saved));
  }
  showToast('Color saved ⭐','ok');
}
