...
// ── Safe wrappers (called from HTML before lazy JS loads) ──
...
async function safeOpenAddProduct(){
  ...
  try {
    ...
    if(typeof openAddProduct!=='function') await new Promise(r=>setTimeout(r,300));
    if(typeof openAddProduct==='function') openAddProduct();
    else showToast('Loading... try again','err');
  } catch(e){ showToast('Try again','err'); }
  finally { if(btn){ btn.disabled=false; btn.textContent='+ Add'; } }
}

function npSuggestCategory(name){
  if(typeof _npSuggestCategory==='function') _npSuggestCategory(name);
}
function epSuggestCategory(name){
  if(typeof _epSuggestCategory==='function') _epSuggestCategory(name);
}
function saveCurrentVColorToCustom(){
  if(typeof _saveCurrentVColorToCustom==='function') _saveCurrentVColorToCustom();
}

// Expenses filter — safe wrapper
function setExpFilter(f){
  window._expStoreFilter = window._expStoreFilter || 'all';
  // Update button styles
  ['all','ra','flora','both'].forEach(v=>{
    const btn = document.getElementById('expf-'+v);
    if(btn){ btn.style.background = v===f ? 'var(--rose)' : 'var(--grey)'; btn.style.color = v===f ? 'white' : 'var(--muted)'; }
  });
  window._expStoreFilter = f;
