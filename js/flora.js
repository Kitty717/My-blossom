...
function floraUpdateStatus(oid, newStatus){
  ...
  const doUpdate = ()=>{
    ...
    // Auto WA on shipped/delivered
    if(newStatus==='shipped'||newStatus==='delivered'){
      const floraWA = (typeof getStoreWA==='function') ? getStoreWA('flora') : '96171664849';
      const custWA = (o.customerPhone||o.wa||'').replace(/\D/g,'');
      if(custWA){
        const msg = newStatus==='shipped'
          ? `Hi ${o.customer||''}! 🌸\n\nYour Flora Gift Shop order is on its way!\nEstimated delivery: 1-3 days 🚚\n\nThank you for your order! 💛`
          : `Hi ${o.customer||''}! 🎉\n\nYour Flora Gift Shop order has been delivered!\nWe hope you love it 🌸\nThank you! 💛`;
        appConfirm('Send WhatsApp?', `Send delivery update to ${o.customer}?`, '💬 Send', ()=>{
          window.open('https://wa.me/'+custWA+'?text='+encodeURIComponent(msg), '_blank');
        });
      } else {
        setTimeout(()=>showWsync(o), 400);
      }
