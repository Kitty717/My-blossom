...
function renderCatalog(){
  const searchQ = (document.getElementById('cat-search')?.value||'').toLowerCase().trim();
  const activeTag = document.querySelector('.cat-tag-pill.active')?.dataset.tag||'';
  renderCatColTabs();
  ...
  const tableRows = (rowsArr)=>rowsArr.map(({p,v})=>{
    ...
    const su = (isRA && p.standUnit && p.standUnit.qty && p.standUnit.price) ? p.standUnit : null;
    return `<tr>
      ...
      ${isRA?`<td class="cat-dz-cell">${su?su.name+' $'+su.price.toFixed(2):'—'}</td>`:''}
    </tr>`;
  }).join('');
    ...
    <div style="border-radius:16px;overflow:hidden;box-shadow:var(--shadow);margin-bottom:6px">
      <table class="cat-table" style="width:100%">
        <thead class="cat-thead"><tr>
          ...
          <th>${isRA?'Price/pc':'Price'}</th>${isRA?'<th>Per Dz</th>':''}${isRA?'<th>Stand</th>':''}
        </tr></thead>
    ...
    prods.forEach(p=>{
      ...
      p.variants.forEach(v=>{
        ...
        if(isRA){
          ...
          const su = p.standUnit&&p.standUnit.qty&&p.standUnit.price ? p.standUnit.name+' $'+p.standUnit.price.toFixed(2)+'('+p.standUnit.qty+'pc)' : '';
          varLines.push(`  • ${vLabel}${[pc,dz,su].filter(Boolean).length?' — '+[pc,dz,su].filter(Boolean).join(' · '):''}`);
        } else {
...
function _renderTagPills(activeTag){
  const el = document.getElementById('cat-tag-pills');
  if(!el) return;
  const allTags = [...new Set(products.flatMap(p=>p.tags||[]))].sort();
  if(!allTags.length){ el.style.display='none'; return; }
  el.style.display='flex';
  el.innerHTML = ['', ...allTags].map(t=>
    `<div class="cat-tag-pill${t===activeTag?' active':''}" data-tag="${t}" onclick="document.querySelectorAll('.cat-tag-pill').forEach(x=>x.classList.remove('active'));this.classList.add('active');renderCatalog()" style="flex-shrink:0;border:none;border-radius:50px;padding:6px 12px;font-size:11px;font-weight:700;cursor:pointer;font-family:inherit;background:${t===activeTag?'var(--rose)':'var(--grey)'};color:${t===activeTag?'white':'var(--muted)'}">
      ${t||'All'}
    </div>`
  ).join('');
}
