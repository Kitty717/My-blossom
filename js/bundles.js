...
function deleteBundle(bid){
  ...
  appConfirm('Delete Bundle',`Delete "${b.name}"?`,'🗑️ Delete',()=>{
    ...
    saveBundles(); renderBundles(); showToast('Bundle deleted');
  });
}
...
function showAutoBundleResult(result, budget, pkg){
  ...
  const itemsHtml = result.items.map((it,idx)=>{
    ...
    const thumbHtml = photo
      ? `<div style="width:48px;height:48px;border-radius:10px;overflow:hidden;flex-shrink:0;border:1.5px solid rgba(0,0,0,0.07)"><img src="${photo}" style="width:100%;height:100%;object-fit:cover"></div>`
      : `<div style="width:48px;height:48px;border-radius:10px;background:var(--rose-pale);display:flex;align-items:center;justify-content:center;font-size:22px;flex-shrink:0">${p?.emoji||'📦'}</div>`;
...
{"productId":"...","variantId":"...","productName":"...","variantName":"...","colorHex":"...","priceFlora":0,"reason":"..."}`;
  ...
  try {
    ...
    const jsonStart = text.indexOf('{');
    const jsonEnd = text.lastIndexOf('}');
    if(jsonStart === -1 || jsonEnd === -1) throw new Error('No JSON in swap response');
    text = text.slice(jsonStart, jsonEnd + 1);
